...
from analytickit.async_migrations.runner import MAX_CONCURRENT_ASYNC_MIGRATIONS, is_analytickit_version_compatible
from analytickit.async_migrations.setup import get_async_migration_definition
from analytickit.async_migrations.utils import force_stop_migration, rollback_migration, trigger_migration
...
from analytickit.permissions import IsStaffUser
...
class AsyncMigrationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            "started_at",
            "finished_at",
            "analytickit_max_version",
            "analytickit_min_version",
            "parameters",
            "error_count",
...
class AsyncMigrationsViewset(StructuredViewSetMixin, viewsets.ModelViewSet):
    ...
    def trigger(self, request, **kwargs):
        ...
        if not is_analytickit_version_compatible(
                migration_instance.analytickit_min_version, migration_instance.analytickit_max_version
        ):
            return response.Response(
                {
                    "success": False,
                    "error": f"Can't run migration. Minimum AnalyticKit version: {migration_instance.analytickit_min_version}. Maximum AnalyticKit version: {migration_instance.analytickit_max_version}",
                },
                status=400,
            )
