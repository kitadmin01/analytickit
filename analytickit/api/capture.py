...
from analytickit.api.utils import (
...
from analytickit.exceptions import generate_exception_response
from analytickit.helpers.session_recording import (
    ChunkedRecordingEvent,
    get_session_recording_events_for_object_storage,
    preprocess_session_recording_events_for_clickhouse,
)
from analytickit.kafka_client.client import KafkaProducer
from analytickit.kafka_client.topics import KAFKA_DEAD_LETTER_QUEUE
from analytickit.logging.timing import timed
from analytickit.models.feature_flag import get_active_feature_flags
...
from analytickit.utils import cors_response, get_ip_address, should_write_recordings_to_object_storage
...
def log_event(data: Dict, event_name: str, partition_key: str) -> None:
    ...
    try:
        ...
        statsd.incr("analytickit_cloud_plugin_server_ingestion")
...
def _get_sent_at(data, request) -> Tuple[Optional[datetime], Any]:
    try:
        if request.GET.get("_"):  # analytickit-js
            sent_at = request.GET["_"]
        elif isinstance(data, dict) and data.get("sent_at"):  # analytickit-android, analytickit-ios
...
@csrf_exempt
@timed("analytickit_cloud_event_endpoint")
def get_event(request):
    ...
    if isinstance(data, dict):
        if data.get("batch"):  # analytickit-python and analytickit-ruby
            data = data["batch"]
    ...
    for event, event_uuid, distinct_id in processed_events:
        ...
        try:
            capture_internal(event, distinct_id, ip, site_url, now, sent_at, ingestion_context.team_id,
                             event_uuid)  # type: ignore
        except Exception as e:
            capture_exception(e, {"data": data})
            statsd.incr(
                "analytickit_cloud_raw_endpoint_failure", tags={"endpoint": "capture", },
            )
            return cors_response(
                request,
                generate_exception_response(
    ...
    statsd.incr(
        "analytickit_cloud_raw_endpoint_success", tags={"endpoint": "capture", },
    )
    return cors_response(request, JsonResponse({"status": 1}))
