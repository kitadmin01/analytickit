...
from analytickit.api.geoip import get_geoip_properties
from analytickit.api.utils import get_token
...
from analytickit.utils import cors_response, get_ip_address, get_js_url, load_data_from_request
...
def get_decide(request: HttpRequest):
    ...
    if request.method == "POST":
        try:
            ...
            # default value added because of bug in analytickit-js 1.19.0
            # see https://sentry.io/organizations/analytickit2/issues/2738865125/?project=1899813
            # as a tombstone if the below statsd counter hasn't seen errors for N days
            # then it is likely that no clients are running analytickit-js 1.19.0
            # and this defaulting could be removed
            statsd.incr(
                f"analytickit_cloud_decide_defaulted_api_version_on_value_error",
    ...
    statsd.incr(
        f"analytickit_cloud_raw_endpoint_success", tags={"endpoint": "decide", },
    )
