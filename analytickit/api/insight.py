...
from analytickit.api.documentation import extend_schema
from analytickit.api.forbid_destroy_model import ForbidDestroyModel
from analytickit.api.insight_serializers import (
    FunnelSerializer,
...
from analytickit.api.tagged_item import TaggedItemSerializerMixin, TaggedItemViewSetMixin
from analytickit.api.utils import format_paginated_url
...
from analytickit.helpers.multi_property_breakdown import protect_old_clients_from_multi_property_default
from analytickit.models import DashboardTile, Filter, Insight, Team, User
from analytickit.models.activity_logging.activity_log import Change, Detail, changes_between, load_activity, \
    log_activity
...
from analytickit.models.filters import RetentionFilter
...
from analytickit.models.insight import InsightViewed, generate_insight_cache_key
...
from analytickit.queries.funnels import ClickhouseFunnelTimeToConvert, ClickhouseFunnelTrends
...
from analytickit.tasks.update_cache import synchronously_update_insight_cache
from analytickit.utils import DEFAULT_DATE_FROM_DAYS, get_safe_cache, relative_date_parse, should_refresh, str_to_bool
...
class InsightSerializer(InsightBasicSerializer):
    ...
    def get_result(self, insight: Insight):
        ...
        if not result or result.get("task_id", None):
            statsd.incr("analytickit_cloud_insight_cache_miss", tags=cache_context)
            return None
        else:
            statsd.incr("analytickit_cloud_insight_cache_hit", tags=cache_context)
...
class InsightViewSet(TaggedItemViewSetMixin, StructuredViewSetMixin, ForbidDestroyModel, viewsets.ModelViewSet):
    ...
    def trend(self, request: request.Request, *args: Any, **kwargs: Any):
        ...
        if self.request.accepted_renderer.format == "csv":
            ...
            response[
                "Content-Disposition"
            ] = 'attachment; filename="{name} ({date_from} {date_to}) from Analytickit.csv"'.format(
