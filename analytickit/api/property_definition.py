...
from analytickit.constants import GROUP_TYPES_LIMIT, AvailableFeature
...
from analytickit.models import PropertyDefinition, TaggedItem
...
class PropertyDefinitionViewSet(
    ...
    def get_queryset(self):
        ...
        if event_names and len(event_names) > 0:
            event_property_field = "(SELECT count(1) > 0 FROM analytickit_eventproperty WHERE analytickit_eventproperty.team_id=analytickit_propertydefinition.team_id AND analytickit_eventproperty.event IN %(event_names)s AND analytickit_eventproperty.property = analytickit_propertydefinition.name)"
        ...
        return PropertyDefinition.objects.raw(
            f"""
                SELECT {property_definition_fields},
                       {event_property_field} AS is_event_property
                FROM analytickit_propertydefinition
                WHERE team_id = %(team_id)s AND name NOT IN %(excluded_properties)s
                {name_filter} {numerical_filter} {search_query} {event_property_filter} {is_feature_flag_filter}
                ORDER BY is_event_property DESC, query_usage_30_day DESC NULLS LAST, name ASC
            """,
