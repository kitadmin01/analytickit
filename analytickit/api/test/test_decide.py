...
from analytickit.models import FeatureFlag, GroupTypeMapping, Person, PersonalAPIKey
...
class TestDecide(BaseTest):
    ...
    def test_defaults_to_v2_if_conflicting_parameters(self):
        """
        regression test for https://sentry.io/organizations/analytickit2/issues/2738865125/?project=1899813
        analytickit-js version 1.19.0 (but not versions before or after)
    ...
    def test_feature_flags(self):
        ...
        self.team.save()
        self.client.logout()
        Person.objects.create(team=self.team, distinct_ids=["example_id"], properties={"email": "tim@analytickit.com"})
        ...
        FeatureFlag.objects.create(
            team=self.team,
            filters={"properties": [{"key": "email", "value": "tim@analytickit.com", "type": "person"}]},
        ...
        FeatureFlag.objects.create(
            team=self.team,
            filters={"groups": [{"properties": [{"key": "email", "value": "tim@analytickit.com", "type": "person"}]}]},
    ...
    def test_feature_flags_v2_consistent_flags(self):
        ...
        person = Person.objects.create(
            team=self.team, distinct_ids=["example_id"], properties={"email": "tim@analytickit.com"}
        )
        FeatureFlag.objects.create(
            team=self.team,
        ...
        # person.delete()
        # person2 = Person.objects.create(team=self.team, distinct_ids=["example_id", "other_id"], properties={"email": "tim@analytickit.com"})
    ...
    def test_feature_flags_v2_consistent_flags_with_ingestion_delays(self):
        ...
        # person = Person.objects.create(team=self.team, distinct_ids=["example_id"], properties={"email": "tim@analytickit.com"})
    ...
    def test_feature_flags_v2_consistent_flags_with_merged_persons(self):
        ...
        person2 = Person.objects.create(
            team=self.team, distinct_ids=["other_id"], properties={"email": "tim@analytickit.com"}
        )
        ...
        # https://github.com/AnalyticKit/analytickit/blob/master/plugin-server/src/worker/ingestion/person-state.ts#L421
        # at which point we run the query
        query = f"""
            UPDATE analytickit_featureflaghashkeyoverride SET person_id = {new_person_id} WHERE person_id = {old_person_id}
        """
    ...
    def test_feature_flags_v2_complex(self):
        ...
        Person.objects.create(
            team=self.team, distinct_ids=["example_id"], properties={"email": "tim@analytickit.com", "realm": "cloud"}
        )
        Person.objects.create(
            team=self.team, distinct_ids=["hosted_id"], properties={"email": "sam@analytickit.com", "realm": "hosted"}
        )
        FeatureFlag.objects.create(
            team=self.team,
            filters={"groups": [{"properties": [], "rollout_percentage": None}]},
    ...
    def test_feature_flags_v2_with_groups(self):
        # More in-depth tests in analytickit/api/test/test_feature_flag.py
