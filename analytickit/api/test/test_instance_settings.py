...
from analytickit.api.instance_settings import get_instance_setting as get_instance_setting_helper
from analytickit.models.instance_setting import get_instance_setting, override_instance_config, set_instance_setting
from analytickit.settings import CONSTANCE_CONFIG
    ...
    def test_updating_email_settings(self):
        ...
        with self.settings(SITE_URL="http://localhost:8000", CELERY_TASK_ALWAYS_EAGER=True):
            response = self.client.patch(
                f"/api/instance_settings/EMAIL_DEFAULT_FROM", {"value": "hellohello@analytickit.com"}
            )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.json()["value"], "hellohello@analytickit.com")
