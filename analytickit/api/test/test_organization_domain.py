...
from analytickit.models import Organization, OrganizationDomain, OrganizationMembership, Team
from analytickit.test.base import APIBaseTest, BaseTest
...
class TestOrganizationDomainsAPI(APIBaseTest):
    ...
    def setUpTestData(cls):
        ...
        cls.domain = OrganizationDomain.objects.create(organization=cls.organization, domain="myanalytickit.com")

        cls.another_org = Organization.objects.create(name="Another Org")
        Team.objects.create(organization=cls.another_org)
        cls.another_domain = OrganizationDomain.objects.create(organization=cls.another_org,
                                                               domain="org.analytickit.net")
    ...
    def test_can_list_and_retrieve_domains(self):
        ...
        self.assertEqual(item["domain"], "myanalytickit.com")
    ...
    def test_create_domain(self):
        ...
        response_data = response.json()
        self.assertEqual(response_data["domain"], "the.analytickit.com")
        ...
        self.assertEqual(instance.domain, "the.analytickit.com")
    ...
    def test_cannot_create_invalid_domain(self):
        ...
        self.organization_membership.level = OrganizationMembership.Level.ADMIN
        self.organization_membership.save()
        invalid_domains = ["test@analytickit.com", "🦔🦔🦔.com", "one.two.c", "--alpha.com", "javascript: alert(1)"]
    ...
    @patch("analytickit.models.organization_domain.dns.resolver.resolve")
    def test_can_request_verification_for_unverified_domains(self, mock_dns_query):
        ...
        mock_dns_query.return_value = FakeDNSResponse(
            [
                dns.rrset.from_text(
                    "_analytickit-challenge.myanalytickit.com.", 3600, "IN", "TXT", self.domain.verification_challenge,
                )
            ],
        )
        ...
        self.assertEqual(response_data["domain"], "myanalytickit.com")
        self.assertEqual(
    ...
    def test_domain_is_not_verified_with_incorrect_challenge(self, mock_dns_query):
        ...
        mock_dns_query.return_value = FakeDNSResponse(
            [dns.rrset.from_text("_analytickit-challenge.myanalytickit.com.", 3600, "IN", "TXT",
                                 "incorrect_challenge", )],
        )
    ...
    def test_only_admin_can_create_verified_domains(self):
        count = OrganizationDomain.objects.count()
        response = self.client.post("/api/organizations/@current/domains/", {"domain": "evil.analytickit.com"})
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        self.assertEqual(
