...
from analytickit.models import Plugin, PluginAttachment, PluginConfig, PluginSourceFile
from analytickit.models.organization import Organization, OrganizationMembership
from analytickit.plugins.access import (
    can_configure_plugins,
    can_globally_manage_plugins,
    can_install_plugins,
    can_view_plugins,
)
from analytickit.plugins.test.mock import mocked_plugin_requests_get
from analytickit.plugins.test.plugin_archives import (
...
@mock.patch("analytickit.models.plugin.reload_plugins_on_workers", side_effect=mocked_plugin_reload)
...
class TestPluginAPI(APIBaseTest):
    ...
    def setUpTestData(cls):
        ...
        # We make sure the org has permissions for these tests, particularly for tests on analytickit-cloud
    ...
    @mock.patch("analytickit.models.plugin.PluginSourceFile.objects.sync_from_plugin_archive")
    ...
    def test_create_plugin_commit_url(self, mock_get, mock_reload):
        ...
        response = self.client.post(
            ...
            {"url": f"https://github.com/AnalyticKit/helloworldplugin/commit/{HELLO_WORLD_PLUGIN_GITHUB_ZIP[0]}"},
        )
        self.assertEqual(response.status_code, 201)
        self.assertEqual(
            response.json(),
    ...
    def test_create_plugin_version_range_eq_current(self, mock_get, mock_reload):
        with self.settings(MULTI_TENANCY=False):
            response = self.client.post(
                "/api/organizations/@current/plugins/",
                {"url": f"https://github.com/AnalyticKit-plugin/version-equals/commit/{VERSION}"},
            )
            self.assertEqual(response.status_code, 201)
    ...
    def test_create_plugin_version_range_eq_next_minor(self, mock_get, mock_reload):
        with self.settings(MULTI_TENANCY=False):
            response = self.client.post(
                "/api/organizations/@current/plugins/",
                {"url": f"https://github.com/AnalyticKit-plugin/version-equals/commit/{Version(VERSION).next_minor()}"},
            )
            self.assertEqual(response.status_code, 400)
            self.assertEqual(
                cast(Dict[str, str], response.json())["detail"],
                f'Currently running AnalyticKit version {VERSION} does not match this plugin\'s semantic version requirement "{Version(VERSION).next_minor()}".',
            )
    ...
    def test_create_plugin_version_range_gt_current(self, mock_get, mock_reload):
        with self.settings(MULTI_TENANCY=False):
            response = self.client.post(
                "/api/organizations/@current/plugins/",
                {"url": f"https://github.com/AnalyticKit-plugin/version-greater-than/commit/0.0.0"},
            )
            self.assertEqual(response.status_code, 201)
    ...
    def test_create_plugin_version_range_gt_next_major(self, mock_get, mock_reload):
        with self.settings(MULTI_TENANCY=False):
            response = self.client.post(
                "/api/organizations/@current/plugins/",
                {
                    "url": f"https://github.com/AnalyticKit-plugin/version-greater-than/commit/{Version(VERSION).next_major()}"
                },
            )
            self.assertEqual(response.status_code, 400)
            self.assertEqual(
                cast(Dict[str, str], response.json())["detail"],
                f'Currently running AnalyticKit version {VERSION} does not match this plugin\'s semantic version requirement ">= {Version(VERSION).next_major()}".',
            )
    ...
    def test_create_plugin_version_range_lt_current(self, mock_get, mock_reload):
        with self.settings(MULTI_TENANCY=False):
            response = self.client.post(
                "/api/organizations/@current/plugins/",
                {"url": f"https://github.com/AnalyticKit-plugin/version-less-than/commit/{VERSION}"},
            )
            self.assertEqual(response.status_code, 400)
            self.assertEqual(
                cast(Dict[str, str], response.json())["detail"],
                f'Currently running AnalyticKit version {VERSION} does not match this plugin\'s semantic version requirement "< {VERSION}".',
            )
    ...
    def test_create_plugin_version_range_lt_invalid(self, mock_get, mock_reload):
        with self.settings(MULTI_TENANCY=False):
            response = self.client.post(
                "/api/organizations/@current/plugins/",
                {"url": f"https://github.com/AnalyticKit-plugin/version-less-than/commit/..."},
            )
            self.assertEqual(response.status_code, 400)
            self.assertEqual(
                cast(Dict[str, str], response.json())["detail"],
    ...
    def test_install_plugin_on_multiple_orgs(self, mock_get, mock_reload):
        ...
        with freeze_time(fake_date.isoformat()):
            response = self.client.post(
                f"/api/organizations/{my_org.id}/plugins/", {"url": "https://github.com/AnalyticKit/helloworldplugin"},
            )
            self.assertEqual(response.status_code, 201)
            self.assertEqual(Plugin.objects.count(), 1)
        ...
        response = self.client.post(
            f"/api/organizations/{other_org.id}/plugins/", {"url": "https://github.com/AnalyticKit/helloworldplugin"},
        )
        self.assertEqual(response.status_code, 403)
        self.assertEqual(Plugin.objects.count(), 1)
    ...
    def test_create_plugin_config_with_secrets(self, mock_get, mock_reload):
        ...
        self.assertEqual(
            response.json(),
            {
                ...
                "enabled": True,
                "order": 0,
                "config": {"bar": "**************** AnalyticKit SECRET FIELD ****************"},
                "error": None,
                "team_id": self.team.pk,
            },
        )
            ...
            [
                {
                    ...
                    "item_id": str(plugin_id),
                    "scope": "Plugin",
                    "user": {"email": "user1@analytickit.com", "first_name": ""},
                },
                {
                    "activity": "enabled",
                    "created_at": "2021-08-25T22:09:14.252000Z",
