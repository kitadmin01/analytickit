...
from analytickit.models import Dashboard, Organization, Team, User
from analytickit.models.instance_setting import override_instance_config
from analytickit.models.organization import OrganizationInvite, OrganizationMembership
from analytickit.models.organization_domain import OrganizationDomain
from analytickit.test.base import APIBaseTest
from analytickit.utils import get_instance_realm

MOCK_GITLAB_SSO_RESPONSE = {
    "access_token": "123",
    "email": "testemail@analytickit.com",
    "name": "John Doe",
}
...
class TestSignupAPI(APIBaseTest):
    ...
    def test_api_sign_up(self, mock_capture):
        ...
        Organization.objects.create(name="AnalyticKit Internal Metrics", for_internal_metrics=True)

        response = self.client.post(
            "/api/signup/",
            {
                "first_name": "John",
                "email": "hedgehog@analytickit.com",
                "password": "notsecure",
        ...
        self.assertEqual(user.email, "hedgehog@analytickit.com")
        ...
        # Assert that the sign up event & identify calls were sent to AnalyticKit analytics
        ...
        self.assertEqual(response.json()["email"], "hedgehog@analytickit.com")
    ...
    def test_signup_disallowed_on_self_hosted_by_default(self):
        with self.settings(MULTI_TENANCY=False):
            response = self.client.post(
                "/api/signup/", {"first_name": "Jane", "email": "hedgehog2@analytickit.com", "password": "notsecure"},
            )
            self.assertEqual(response.status_code, status.HTTP_201_CREATED)
            response = self.client.post(
                "/api/signup/", {"first_name": "Jane", "email": "hedgehog2@analytickit.com", "password": "notsecure"},
    ...
    def test_signup_allowed_on_self_hosted_with_env_var(self):
        try:
            from dpa.models.license import License, LicenseManager
        except ImportError:
            pass
        else:
            ...
            User.objects.create(first_name="name", email="email@analytickit.com")
            count = Organization.objects.count()
            with self.settings(MULTI_TENANCY=False, MULTI_ORG_ENABLED=True):
                response = self.client.post(
                    "/api/signup/",
                    {"first_name": "Jane", "email": "hedgehog4@analytickit.com", "password": "notsecure"},
                )
            self.assertEqual(response.status_code, status.HTTP_201_CREATED)
            self.assertEqual(response.json()["email"], "hedgehog4@analytickit.com")
            self.assertEqual(Organization.objects.count(), count + 1)

    @pytest.mark.skip_on_multitenancy
    @patch("analytickitanalytics.capture")
    @patch("analytickitanalytics.identify")
    def test_signup_minimum_attrs(self, mock_identify, mock_capture):
        ...
        self.assertEqual(
            response.json(),
            {
                ...
                "distinct_id": user.distinct_id,
                "first_name": "Jane",
                "email": "hedgehog2@analytickit.com",
                "redirect_url": "/ingestion",
            },
        )
        ...
        self.assertEqual(user.email, "hedgehog2@analytickit.com")
        ...
        response = self.client.get("/api/users/@me/")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.json()["email"], "hedgehog2@analytickit.com")
    ...
    def test_cant_sign_up_with_short_password(self):
        ...
        response = self.client.post(
            "/api/signup/", {"first_name": "Jane", "email": "failed@analytickit.com", "password": "123"},
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(
            response.json(),
    ...
    def test_default_dashboard_is_created_on_signup(self):
        ...
        response = self.client.post(
            "/api/signup/", {"first_name": "Jane", "email": "hedgehog75@analytickit.com", "password": "notsecure",},
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        ...
        self.assertEqual(
            response.json(),
            {
                ...
                "distinct_id": user.distinct_id,
                "first_name": "Jane",
                "email": "hedgehog75@analytickit.com",
                "redirect_url": "/ingestion",
            },
        )
    ...
    def test_api_can_use_social_login_to_create_organization_if_enabled(self, mock_request):
        ...
        try:
            ...
            self.assertRedirects(
                response,
                "/organization/confirm-creation?organization_name=&first_name=John%20Doe&email=testemail%40analytickit.com",
    ...
    def test_api_social_login_to_create_organization(self, mock_request):
        ...
        self.assertRedirects(
            response,
            "/organization/confirm-creation?organization_name=HogFlix&first_name=John%20Doe&email=testemail%40analytickit.com",
        )  # page where user will create a new org

    @mock.patch("social_core.backends.base.BaseAuth.request")
    @mock.patch("analytickit.api.authentication.get_instance_available_sso_providers")
    @pytest.mark.skip_on_multitenancy
    ...
    def run_test_for_whitelisted_domain(self, mock_sso_providers, mock_request):
        ...
        OrganizationDomain.objects.create(
            domain="hogflix.analytickit.com",
        ...
        mock_request.return_value.json.return_value = {
            "access_token": "123",
            "email": "jane@hogflix.analytickit.com",
        }
        ...
        self.assertEqual(user.email, "jane@hogflix.analytickit.com")
    ...
    def test_social_signup_with_whitelisted_domain_on_cloud_reverse(self, mock_sso_providers, mock_request):
        with self.settings(MULTI_TENANCY=True):
            # user already exists
            User.objects.create(email="jane@hogflix.analytickit.com", distinct_id=str(uuid.uuid4()))
    ...
    def test_cannot_social_signup_with_whitelisted_but_jit_provisioning_disabled(
        ...
        OrganizationDomain.objects.create(
            domain="analytickit.net", verified_at=timezone.now(), jit_provisioning_enabled=False, organization=new_org,
        ...
        url = reverse("social:complete", kwargs={"backend": "google-oauth2"})
        url += f"?code=2&state={response.client.session['google-oauth2_state']}"
        mock_request.return_value.json.return_value = {"access_token": "123", "email": "alice@analytickit.net"}
    ...
    def test_cannot_social_signup_with_whitelisted_but_unverified_domain(self, mock_sso_providers, mock_request):
        ...
        new_org = Organization.objects.create(name="Test org")
        OrganizationDomain.objects.create(
            domain="analytickit.net", verified_at=None, jit_provisioning_enabled=True, organization=new_org,
    ...
    def test_social_signup_to_existing_org_without_whitelisted_domain_on_cloud(self, mock_sso_providers, mock_request):
        ...
        url = reverse("social:complete", kwargs={"backend": "google-oauth2"})
        url += f"?code=2&state={response.client.session['google-oauth2_state']}"
        mock_request.return_value.json.return_value = {"access_token": "123", "email": "jane@hogflix.analytickit.com"}
        ...
        self.assertRedirects(
            response,
            "/organization/confirm-creation?organization_name=&first_name=jane&email=jane%40hogflix.analytickit.com",
        )  # page where user will create a new org
...
class TestInviteSignup(APIBaseTest):
    ...
    def test_api_invite_sign_up_prevalidate(self):
        invite: OrganizationInvite = OrganizationInvite.objects.create(
            target_email="test+19@analytickit.com", organization=self.organization,
        )
        ...
        self.assertEqual(
            ...
            {
                "id": str(invite.id),
                "target_email": "t*****9@analytickit.com",
                "first_name": "",
    ...
    def test_api_invite_sign_up_with_first_name_prevalidate(self):
        invite: OrganizationInvite = OrganizationInvite.objects.create(
            target_email="test+58@analytickit.com", organization=self.organization, first_name="Jane"
        )
        ...
        self.assertEqual(
            ...
            {
                "id": str(invite.id),
                "target_email": "t*****8@analytickit.com",
                "first_name": "Jane",
                "organization_name": self.CONFIG_ORGANIZATION_NAME,
            },
        )
    ...
    def test_api_invite_sign_up_prevalidate_for_existing_user(self):
        user = self._create_user("test+29@analytickit.com", "test_password")
        new_org = Organization.objects.create(name="Test, Inc")
        invite: OrganizationInvite = OrganizationInvite.objects.create(
            target_email="test+29@analytickit.com", organization=new_org,
        )
    ...
    def test_existing_user_cant_claim_invite_if_it_doesnt_match_target_email(self):
        user = self._create_user("test+39@analytickit.com", "test_password")
        invite: OrganizationInvite = OrganizationInvite.objects.create(
            target_email="test+49@analytickit.com", organization=self.organization,
        )
        ...
        self.assertEqual(
            response.json(),
            {
                ...
                "detail": "This invite is intended for another email address: t*****9@analytickit.com."
                " You tried to sign up with test+39@analytickit.com.",
                "attr": None,
            },
        )
    ...
    def test_api_invite_sign_up_prevalidate_expired_invite(self):
        invite: OrganizationInvite = OrganizationInvite.objects.create(
            target_email="test+59@analytickit.com", organization=self.organization,
        )
    ...
    def test_api_invite_sign_up(self, mock_capture):
        invite: OrganizationInvite = OrganizationInvite.objects.create(
            target_email="test+99@analytickit.com", organization=self.organization,
        )
        ...
        self.assertEqual(user.email, "test+99@analytickit.com")
        ...
        response = self.client.get("/api/users/@me/")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.json()["email"], "test+99@analytickit.com")
    ...
    def test_api_invite_sign_up_where_there_are_no_default_non_private_projects(self):
        self.client.logout()
        invite: OrganizationInvite = OrganizationInvite.objects.create(
            target_email="test+private@analytickit.com", organization=self.organization,
        )
    ...
    def test_api_invite_sign_up_where_default_project_is_private(self):
        ...
        invite: OrganizationInvite = OrganizationInvite.objects.create(
            target_email="test+privatepublic@analytickit.com", organization=self.organization,
        )
        response = self.client.post(
    ...
    def test_api_invite_sign_up_member_joined_email_is_not_sent_for_initial_member(self):
        invite: OrganizationInvite = OrganizationInvite.objects.create(
            target_email="test+100@analytickit.com", organization=self.organization,
        )

        with self.settings(EMAIL_ENABLED=True, EMAIL_HOST="localhost", SITE_URL="http://test.analytickit.com"):
            response = self.client.post(
                f"/api/signup/{invite.id}/", {"first_name": "Alice", "password": "test_password", "email_opt_in": True},
            )
    ...
    def test_api_invite_sign_up_member_joined_email_is_sent_for_next_members(self):
        with override_instance_config("EMAIL_HOST", "localhost"):
            initial_user = User.objects.create_and_join(self.organization, "test+420@analytickit.com", None)

            invite: OrganizationInvite = OrganizationInvite.objects.create(
                target_email="test+100@analytickit.com", organization=self.organization,
            )

            with self.settings(EMAIL_ENABLED=True, SITE_URL="http://test.analytickit.com"):
                response = self.client.post(
    ...
    def test_api_invite_sign_up_member_joined_email_is_not_sent_if_disabled(self):
        ...
        User.objects.create_and_join(self.organization, "test+420@analytickit.com", None)
    ...
    def test_existing_user_can_sign_up_to_a_new_organization(self, mock_capture, mock_identify):
        user = self._create_user("test+159@analytickit.com", "test_password")
        new_org = Organization.objects.create(name="TestCo")
        new_team = Team.objects.create(organization=new_org)
        invite: OrganizationInvite = OrganizationInvite.objects.create(
            target_email="test+159@analytickit.com", organization=new_org,
        )
        ...
        self.assertEqual(
            response.json(),
            {
                ...
                "distinct_id": user.distinct_id,
                "first_name": "",
                "email": "test+159@analytickit.com",
            },
        )
        ...
        self.assertEqual(user.email, "test+159@analytickit.com")
    ...
    def test_cannot_use_claim_invite_endpoint_to_update_user(self, mock_capture):
        ...
        """
        new_org = Organization.objects.create(name="TestCo")
        user = self._create_user("test+189@analytickit.com", "test_password")
        user2 = self._create_user("test+949@analytickit.com")
        user2.join(organization=new_org)

        Team.objects.create(organization=new_org)
        invite: OrganizationInvite = OrganizationInvite.objects.create(
            target_email="test+189@analytickit.com", organization=new_org,
        )
        ...
        self.assertEqual(
            response.json(),
            {
                ...
                "distinct_id": user.distinct_id,
                "first_name": "",
                "email": "test+189@analytickit.com",
    ...
    def test_cant_claim_sign_up_invite_without_required_attributes(self):
        ...
        invite: OrganizationInvite = OrganizationInvite.objects.create(
            target_email="test+799@analytickit.com", organization=self.organization,
        )
    ...
    def test_api_social_invite_sign_up(self):
        ...
        session = self.client.session
        session.update({"backend": "google-oauth2", "email": "test_api_social_invite_sign_up@analytickit.com"})
        session.save()
        ...
        self.assertEqual(
            User.objects.filter(email="test_api_social_invite_sign_up@analytickit.com", first_name="Max").count(), 1
        )
