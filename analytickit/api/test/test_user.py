...
from analytickit.models import Tag, Team, User
...
class TestUserAPI(APIBaseTest):
    ...
    def test_can_only_list_yourself(self):
        ...
        user = self._create_user("newtest@analytickit.com")
    ...
    def test_creating_users_on_this_endpoint_is_not_supported(self):
        ...
        response = self.client.post("/api/users/", {"first_name": "James", "email": "test+james@analytickit.com"})
    ...
    def test_update_current_user(self, mock_capture):
        ...
        user = self._create_user("old@analytickit.com", password="12345678")
        self.client.force_login(user)
        response = self.client.patch(
            "/api/users/@me/",
            {
                "first_name": "Cooper",
                "email": "updated@analytickit.com",
                "anonymize_data": True,
                "email_opt_in": False,
        ...
        self.assertEqual(response_data["email"], "updated@analytickit.com")
        ...
        self.assertEqual(user.email, "updated@analytickit.com")
    ...
    def test_user_can_update_password(self, mock_capture):
        ...
        user = self._create_user("bob@analytickit.com", password="A12345678")
        self.client.force_login(user)

        response = self.client.patch("/api/users/@me/", {"current_password": "A12345678", "password": "a_new_password"})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        response_data = response.json()
        self.assertEqual(response_data["email"], "bob@analytickit.com")
        self.assertNotIn("password", response_data)
        self.assertNotIn("current_password", response_data)
        ...
        response = self.client.post("/api/login", {"email": "bob@analytickit.com", "password": "a_new_password"})
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    @patch("analytickitanalytics.capture")
    def test_user_with_no_password_set_can_set_password(self, mock_capture):
        user = self._create_user("no_password@analytickit.com", password=None)
        self.client.force_login(user)
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        response_data = response.json()
        self.assertEqual(response_data["email"], "no_password@analytickit.com")
        self.assertNotIn("password", response_data)
        self.assertNotIn("current_password", response_data)
    ...
    def test_user_with_unusable_password_set_can_set_password(self):
        user = self._create_user("no_password@analytickit.com", password="123456789")
    ...
    def test_unauthenticated_user_cannot_update_anything(self):
        ...
        self.user.refresh_from_db()
        self.assertNotEqual(self.user.email, "new@analytickit.com")
    ...
    @patch("analytickit.api.user.secrets.token_urlsafe")
...
class TestStaffUserAPI(APIBaseTest):
    ...
    def test_update_staff_user(self):
        user = self._create_user("newuser@analytickit.com", password="12345678")
