...
from analytickit.api.test.test_capture import mocked_get_ingest_context_from_token
...
class TestUtils(BaseTest):
    def test_get_team(self):
        ...
        self.assertEqual("Project API key invalid" in json.loads(error_response.getvalue())["detail"],
                         True)  # type: ignore
        ...
        get_team_from_token_patcher = patch(
            "analytickit.api.utils.get_event_ingestion_context_for_token",
    ...
    def test_get_target_entity(self):
        request = lambda url: cast(Any, RequestFactory().get(url))
        filter = Filter(
            data={"entity_id": "$pageview", "entity_type": "events",
    ...
    # keep in sync with analytickit/plugin-server/tests/utils.test.ts::safeClickhouseString
