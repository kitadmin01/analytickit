...
from analytickit.api.organization import OrganizationSerializer
from analytickit.api.shared import OrganizationBasicSerializer, TeamBasicSerializer
from analytickit.auth import authenticate_secondarily
from analytickit.event_usage import report_user_updated
...
def redirect_to_site(request):
    ...
    # see https://github.com/AnalyticKit/analytickit/issues/9671
    state = urllib.parse.quote(json.dumps(params), safe="")

    if use_new_toolbar:
        return redirect("{}#__analytickit={}".format(app_url, state))
    else:
...
def test_slack_webhook(request):
    ...
    message = {"text": "_Greetings_ from AnalyticKit!"}
    try:
