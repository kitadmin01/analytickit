...
from analytickit.constants import CombinedEventType
from analytickit.exceptions import RequestParsingError, generate_exception_response
from analytickit.models import Action, Entity, EventDefinition
from analytickit.models.entity import MATH_TYPE
...
from analytickit.utils import cors_response, load_data_from_request
        ...
        if request.POST.get("api_key"):
            ...
            if isinstance(data, dict):
                if data.get("$token"):
                    ...
                    token = data["api_key"]  # server-side libraries like analytickit-python and analytickit-ruby
...
class EventIngestionContext:
    ...
    `analytickit.models.Team`, which includes many variables that are not specific
...
# keep in sync with analytickit/plugin-server/src/utils/db/utils.ts::safeClickhouseString
...
def create_event_definitions_sql(
    ...
    def select_event_definitions(fields: str):
        return f"""
            SELECT {fields} FROM analytickit_eventdefinition
        """
    ...
    if event_type == CombinedEventType.ACTION_EVENT:
        return f"""
            SELECT {raw_action_fields} FROM analytickit_action
            {shared_conditions} AND analytickit_action.deleted = false
            {ordering}
        """
