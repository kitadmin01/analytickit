...
from analytickit.async_migrations.runner import start_async_migration
...
from analytickit.models.person.sql import KAFKA_PERSONS_TABLE_SQL, PERSONS_TABLE_MV_SQL, PERSONS_TABLE_SQL
from analytickit.models.signals import mute_selected_signals
...
CREATE TABLE analytickit_test.person ON CLUSTER '{settings.CLICKHOUSE_CLUSTER}'
(
    `id` UUID,
...
ENGINE = ReplicatedReplacingMergeTree('/clickhouse/tables/{{uuid}}-noshard/analytickit.person', '{{replica}}-{{shard}}', _timestamp)
ORDER BY (team_id, id)
...
class Test0005PersonCollapsedByVersion(AsyncMigrationBaseTest, ClickhouseTestMixin):
    ...
    def recreate_person_table(self, sql=ORIGINAL_TABLE_SQL):
        reload_migration_definitions()
        get_client().delete("analytickit.async_migrations.0005.highwatermark")
    ...
    def test_migration_schema(self):
        setup_async_migrations(ignore_analytickit_version=True)
        migration_successful = start_async_migration(MIGRATION_NAME, ignore_analytickit_version=True)
