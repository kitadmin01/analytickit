...
from analytickit.async_migrations.definition import AsyncMigrationOperation
from analytickit.async_migrations.setup import DEPENDENCY_TO_ASYNC_MIGRATION
from analytickit.celery import app
from analytickit.client import make_ch_pool, sync_execute
from analytickit.email import is_email_available
from analytickit.models.async_migration import AsyncMigration, AsyncMigrationError, MigrationStatus
...
def send_analytics_to_analytickit(event, data):
    analytickitanalytics.project_api_key = "sTMFPsFhdP1Ssg"
...
def _get_all_shard_connections():
    from analytickit.client import ch_pool as default_ch_pool
...
def process_error(
    ...
    if alert:
        if async_migrations_emails_enabled():
            from analytickit.tasks.email import send_async_migration_errored_email
    ...
    from analytickit.async_migrations.runner import attempt_migration_rollback
...
def trigger_migration(migration_instance: AsyncMigration, fresh_start: bool = True):
    from analytickit.tasks.async_migrations import run_async_migration
...
def force_stop_migration(
    ...
    2. Our Celery tasks are not essential for the functioning of analytickit, meaning losing a task is not the end of the world
    """
...
def complete_migration(migration_instance: AsyncMigration, email: bool = True):
    ...
    if needs_update:
        ...
        send_analytics_to_analytickit("Async migration completed", {"name": migration_instance.name})

        if email and async_migrations_emails_enabled():
            from analytickit.tasks.email import send_async_migration_complete_email
