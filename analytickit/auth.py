...
from analytickit.jwt import AnalytickitJwtAudience, decode_jwt
...
class PersonalAPIKeyAuthentication(authentication.BaseAuthentication):
    ...
    def authenticate(cls, request: Union[HttpRequest, Request]) -> Optional[Tuple[Any, None]]:
        ...
        PersonalAPIKey = apps.get_model(app_label="analytickit", model_name="PersonalAPIKey")
        try:
            ...
            if not request.GET.get("temporary_token"):
                raise AuthenticationFailed(
                    detail="No temporary_token set. "
                           ...
                           + "See https://analytickit.com/docs/deployment/running-behind-proxy for more information."
                )
        if request.GET.get("temporary_token"):
            User = apps.get_model(app_label="analytickit", model_name="User")
        ...
        if "HTTP_AUTHORIZATION" in request.META:
            ...
            if authorization_match:
                try:
                    ...
                    info = decode_jwt(token, AnalytickitJwtAudience.IMPERSONATED_USER)
