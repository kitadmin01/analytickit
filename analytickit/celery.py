...
def receiver_setup_logging(loglevel, logfile, format, colorize, **kwargs) -> None:
    ...
    from analytickit.settings import logs
...
def on_worker_start(**kwargs) -> None:
    from analytickit.settings import sentry_init
...
def setup_periodic_tasks(sender: Celery, **kwargs):
    ...
    # Cloud (analytickit-cloud) cron jobs
    ...
    if settings.EE_AVAILABLE:
        ...
        )  # every day at a random minute past midnight. Randomize to avoid overloading license.analytickit.com
...
@app.task(ignore_result=True)
def redis_heartbeat():
    get_client().set("ANALYTICKIT_HEARTBEAT", int(time.time()))
...
def enqueue_clickhouse_execute_with_progress(
    ...
    """
    from analytickit.client import execute_with_progress
...
def pg_table_cache_hit_rate():
    from analytickit.internal_metrics import gauge
...
def clickhouse_lag():
    ...
    for table in CLICKHOUSE_TABLES:
        try:
            ...
            gauge("analytickit_celery_clickhouse__table_lag_seconds", lag, tags={"table": table})
        except:
            pass
...
def ingestion_lag():
    ...
    # Requires https://github.com/AnalyticKit/analytickit-heartbeat-plugin to be enabled on team 2
    ...
    for event, metric in HEARTBEAT_EVENT_TO_INGESTION_LAG_METRIC.items():
        try:
            ...
            gauge(f"analytickit_celery_{metric}_lag_seconds_rough_minute_precision", lag)
        except:
            pass
...
def clickhouse_row_count():
    ...
    for table in CLICKHOUSE_TABLES:
        try:
            ...
            gauge(f"analytickit_celery_clickhouse_table_row_count", rows, tags={"table": table})
        except:
            pass
...
def clickhouse_clear_removed_data():
    from analytickit.models.team.util import delete_clickhouse_data_for_deleted_teams
...
def redis_celery_queue_depth():
    ...
    try:
        ...
        gauge(f"analytickit_celery_queue_depth", llen)
    except:
...
@app.task(ignore_result=True)
def status_report():
    from analytickit.tasks.status_report import status_report
...
def monitoring_check_clickhouse_schema_drift():
    from analytickit.tasks.check_clickhouse_schema_drift import check_clickhouse_schema_drift
...
def calculate_cohort():
    from analytickit.tasks.calculate_cohort import calculate_cohorts
...
def check_cached_items():
    from analytickit.tasks.update_cache import update_cached_items
...
def update_cache_item_task(key: str, cache_type, payload: dict) -> List[Dict[str, Any]]:
    ...
    """
    from analytickit.tasks.update_cache import update_cache_item
...
def calculate_cohort_ids_in_feature_flags_task():
    from analytickit.tasks.cohorts_in_feature_flag import calculate_cohort_ids_in_feature_flags
...
def calculate_event_property_usage():
    from analytickit.tasks.calculate_event_property_usage import calculate_event_property_usage
...
def send_first_ingestion_reminder_emails():
    from analytickit.tasks.email import send_first_ingestion_reminder_emails
...
def send_second_ingestion_reminder_emails():
    from analytickit.tasks.email import send_second_ingestion_reminder_emails
...
def sync_all_organization_available_features():
    from analytickit.tasks.sync_all_organization_available_features import sync_all_organization_available_features
...
def check_async_migration_health():
    from analytickit.tasks.async_migrations import check_async_migration_health
...
def verify_persons_data_in_sync():
    from analytickit.tasks.verify_persons_data_in_sync import verify_persons_data_in_sync as verify
