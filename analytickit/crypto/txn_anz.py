...
class TxnAnalyzer:
    ...
    def perform_campaign_analysis(self):
        ...
        logger.info('total comengs=',com_engs[0])
        s3_keys =  S3File.get_all_key_names()
        # filtered_keys used for testing only
        filtered_keys = [
            key for key in s3_keys 
            if datetime.strptime(key.split('date=')[1].split('/')[0], "%Y-%m-%d") >= datetime(2023, 8, 27)
        ]
        ...
        for campaign in com_engs:
            ...
            logger.info("eth_jsons=", eth_jsons)
            ...
            logger.info("camp_analytic.active_users=",camp_analytic.active_users)
            camp_analytic.save()
    ...
    def store_keys_for_next_day(self):
        ...
        if latest_s3file := S3File.objects.order_by('-creation_ts').first():
            ...
            new_files = self.s3_retriever.fetch_files_for_date(next_date.date())

            for file_name in new_files:
                S3File.objects.get_or_create(file_name=file_name)
