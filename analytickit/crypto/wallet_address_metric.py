    ...
    def calculate_token_holdings_and_transfers(self):
        ...
        for wallet_address in wallet_addresses:
            ...
            for transfer in token_transfers:
                ...
                # Check if the wallet address is the sender or receiver (case-insensitive)
                wallet_addr_lower = wallet_address.visitor_wallet_address.lower()
                if wallet_addr_lower == transfer["from_address"].lower():
                    # Outgoing transfer - subtract value
                    token_details.setdefault(token_address, 0)
                    token_details[token_address] -= value
                elif wallet_addr_lower == transfer["to_address"].lower():
