...
from analytickit.client import query_with_columns, sync_execute
from analytickit.demo.graphile import GraphileJob, bulk_queue_graphile_jobs, copy_graphile_jobs_between_teams
...
from analytickit.tasks.calculate_event_property_usage import calculate_event_property_usage_for_team
...
class MatrixManager:
    ...
    def _prepare_master_team(cls):
        if not Team.objects.filter(id=cls.MASTER_TEAM_ID).exists():
            organization = Organization.objects.create(name="analytickit")
    ...
    def _copy_analytics_data_from_master_team(cls, target_team: Team):
        from analytickit.models.event.sql import COPY_EVENTS_BETWEEN_TEAMS
        from analytickit.models.group.sql import COPY_GROUPS_BETWEEN_TEAMS
        from analytickit.models.person.sql import COPY_PERSON_DISTINCT_ID2S_BETWEEN_TEAMS, COPY_PERSONS_BETWEEN_TEAMS
    ...
    def _sync_postgres_with_clickhouse_data(cls, target_team: Team):
        from analytickit.models.group.sql import SELECT_GROUPS_OF_TEAM
        from analytickit.models.person.sql import SELECT_PERSON_DISTINCT_ID2S_OF_TEAM, SELECT_PERSONS_OF_TEAM
    ...
    def _save_sim_person(cls, team: Team, subject: SimPerson):
        ...
        if subject.past_events:
            from analytickit.models.person.util import create_person, create_person_distinct_id
    ...
    def _save_past_sim_events(team: Team, events: List[SimEvent]):
        ...
        from analytickit.models.event.util import create_event
    ...
    def _save_sim_group(
        ...
        from analytickit.models.group.util import raw_create_group_ch

        raw_create_group_ch(team.pk, type_index, key, properties, timestamp)

    @classmethod
    def _is_demo_data_pre_saved(cls) -> bool:
        from analytickit.models.event.sql import GET_TOTAL_EVENTS_VOLUME
