...
if TYPE_CHECKING:
    from analytickit.demo.matrix.matrix import Cluster, Matrix
...
class SimClient(ABC):
    """An abstract analytickit client."""
...
class SimBrowserClient(SimClient):
    ...
    def capture(self, event: str, properties: Optional[Properties] = None):
        """Capture an arbitrary event. Similar to JS `analytickit.capture()`."""
        combined_properties: Properties = {
    ...
    def identify(self, distinct_id: Optional[str], set_properties: Optional[Properties] = None):
        """Identify person in active client. Similar to JS `analytickit.identify()`.

        Use with distinct_id=None for `analytickit.people.set()`-like behavior."""
    ...
    def group(self, group_type: str, group_key: str, set_properties: Optional[Properties] = None):
        """Link the person to the specified group. Similar to JS `analytickit.group()`."""
        if set_properties is None:
            set_properties = {}
    ...
    def reset(self):
        """Reset active client, for instance when the user logs out. Similar to JS `analytickit.reset()`."""
        self.active_distinct_id = self.device_id
        self.is_logged_in = True

    def register(self, super_properties: Properties):
        """Register super properties. Similar to JS `analytickit.register()`."""
        self.super_properties.update(super_properties)

    def unregister(self, *super_property_keys: str):
        """Removes super properties. Similar to JS `analytickit.unregister()`."""
