...
def report_user_signed_up(
    ...
    analytickitanalytics.identify(user.distinct_id, props)
...
def alias_invite_id(user: User, invite_id: str) -> None:
    analytickitanalytics.alias(f"invite_{invite_id}", user.distinct_id)
...
def report_user_logged_in(
        user: User, social_provider: str = "",
        # which third-party provider processed the login (empty = no third-party)
) -> None:
    """
    Reports that a user has logged in to AnalyticKit.
    """
    analytickitanalytics.capture(
        user.distinct_id,
        "user logged in",
...
def report_org_usage(organization_id: str, distinct_id: str, properties: Dict[str, Any]) -> None:
    ...
    analytickitanalytics.group_identify("organization", organization_id, properties)
