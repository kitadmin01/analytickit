...
def test_compression_and_chunking(raw_snapshot_events, mocker: MockerFixture):
    mocker.patch("analytickit.models.utils.UUIDT", return_value="0178495e-8521-0000-8e1c-2652fa57099b")
...
def test_decompress_uncompressed_events_returns_unmodified_events(raw_snapshot_events):
    ...
    assert (
            decompress_chunked_snapshot_data(1, "someid",
                                             snapshot_data_tagged_with_window_id).snapshot_data_by_window_id[
                "1"
            ]
            == raw_snapshot_data
    )
