...
from analytickit.utils import print_warning
...
def handle_check(necessary_migrations: Sequence[AsyncMigration]):
        ...
        print_warning(
            [
                ...
                *(f"- {migration.get_name_with_requirements()}" for migration in necessary_migrations),
                "See more in Docs: https://analytickit.com/docs/self-host/configure/async-migrations/overview",
            ],
            top_emoji="💥",
            bottom_emoji="💥",
        )
    ...
    if running_migrations.exists():
        print_warning(
            [
                "Stopping analytickit!",
                f"Async migration {running_migrations[0].name} is currently running. If you're trying to update analytickit, wait for it to finish before proceeding",
                "See more in Docs: https://analytickit.com/docs/self-host/configure/async-migrations/overview",
            ],
            top_emoji="⏳",
            bottom_emoji="⏳",
    ...
    if errored_migrations.exists():
        print_warning(
            [
                f"Stopping analytickit!",
                "Some async migrations are currently in an 'Errored' state. If you're trying to update analytickit, please make sure they complete successfully first:",
...
def handle_run(necessary_migrations: Sequence[AsyncMigration]):
    for migration in necessary_migrations:
        ...
        started_successfully = start_async_migration(migration.name, ignore_analytickit_version=True)
