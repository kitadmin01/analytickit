...
from analytickit.api.decide import get_decide
from analytickit.internal_metrics import incr
from analytickit.models import Action, Cohort, Dashboard, FeatureFlag, Insight, Team, User
...
class AllowIPMiddleware:
    ...
    def __call__(self, request: HttpRequest):
        ...
        if ip and any(ip_address(ip) in ip_network(block, strict=False) for block in self.ip_blocks):
            return response
        return HttpResponse(
            "Your IP is not allowed. Check your ALLOWED_IP_BLOCKS settings. If you are behind a proxy, you need to set TRUSTED_PROXIES. See https://analytickit.com/docs/deployment/running-behind-proxy",
            status=403,
        )
...
class AutoProjectMiddleware:
    ...
    Sometimes you get sent a link to AnalyticKit that points to an item from a different project than the one you currently
        ...
        if current_team is not None and not target_queryset.filter(team=current_team).exists():
            ...
            if actual_item is not None:
                ...
                if actual_item_team.get_effective_membership_level(user.id) is not None:
                    ...
                    user.save()
                    # Information for ANALYTICKIT_APP_CONTEXT
