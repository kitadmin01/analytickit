...
class AsyncMigration(models.Model):
    ...
    analytickit_min_version: models.CharField = models.CharField(max_length=20, null=True, blank=True)
    analytickit_max_version: models.CharField = models.CharField(max_length=20, null=True, blank=True)

    parameters: models.JSONField = models.JSONField(default=dict)

    def get_name_with_requirements(self) -> str:
        return (
            f"{self.name} - must be ran on analytickit version {self.analytickit_min_version} up to {self.analytickit_max_version}"
        )
