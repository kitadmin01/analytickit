...
from analytickit.models.property import BehavioralPropertyType, Property, PropertyGroup
...
DELETE_QUERY = """
DELETE FROM "analytickit_cohortpeople" WHERE "cohort_id" = {cohort_id}
"""

UPDATE_QUERY = """
INSERT INTO "analytickit_cohortpeople" ("person_id", "cohort_id", "version")
{values_query}
...
class Cohort(models.Model):
    ...
    def calculate_people_ch(self, pending_version):
        from analytickit.models.cohort.util import recalculate_cohortpeople
        from analytickit.tasks.cohorts_in_feature_flag import get_cohort_ids_in_feature_flags
    ...
    def insert_users_by_list(self, items: List[str]) -> None:
        ...
        """
        batchsize = 1000
        from analytickit.models.cohort.util import insert_static_cohort

        if TEST:
            from analytickit.test.base import flush_persons_and_events
            ...
            for i in range(0, len(items), batchsize):
                ...
                query = UPDATE_QUERY.format(
                    ...
                    values_query=sql.replace(
                        'FROM "analytickit_person"', f', {self.pk}, {self.version or "NULL"} FROM "analytickit_person"',
                        1,
                    ),
                )
    ...
    def _clickhouse_persons_query(self, batch_size=10000, offset=0):
        from analytickit.models.cohort.util import get_person_ids_by_cohort_id
