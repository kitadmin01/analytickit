...
from analytickit.models.property import GroupTypeIndex, PropertyGroup

if TYPE_CHECKING:  # Avoid circular import
    from analytickit.models import Property, Team
...
class SimplifyFilterMixin:
    ...
    def simplify(self: T, team: "Team", **kwargs) -> T:
        ...
        if hasattr(result, "entities_to_dict"):
            for entity_type, entities in result.entities_to_dict().items():
                updated_entities[entity_type] = [self._simplify_entity(team, entity_type, entity, **kwargs) for entity
                                                 in entities]  # type: ignore

        prop_group = self._clear_excess_levels(self._simplify_property_group(team, result.property_groups, **kwargs),
    ...
    def _simplify_entity(
        ...
        from analytickit.models.entity import Entity, ExclusionEntity
    ...
    def _simplify_property(self, team: "Team", property: "Property", **kwargs) -> "PropertyGroup":
        if property.type == "cohort":
            from analytickit.models import Cohort
            from analytickit.models.cohort.util import simplified_cohort_filter_properties
    ...
    def _group_set_property(self, group_type_index: GroupTypeIndex) -> "Property":
        from analytickit.models.property import Property
