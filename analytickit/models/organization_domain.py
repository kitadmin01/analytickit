...
from analytickit.models import Organization
from analytickit.models.utils import UUIDModel
from analytickit.utils import get_instance_available_sso_providers
...
class OrganizationDomain(UUIDModel):
    ...
    )  # verification (through DNS) is only used for analytickit Cloud; on self-hosted we take all domains as verified
    ...
    )  # currently only used for analytickit Cloud; SSO enforcement on self-hosted is set by env var
    ...
    def attempt_verification(self) -> Tuple["OrganizationDomain", bool]:
        ...
        try:
            ...
            dns_response = dns.resolver.resolve(f"_analytickit-challenge.{self.domain}", "TXT")
