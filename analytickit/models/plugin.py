...
from analytickit.plugins.access import can_configure_plugins, can_install_plugins
from analytickit.plugins.reload import reload_plugins_on_workers
...
def update_validated_data_from_url(validated_data: Dict[str, Any], url: str) -> Dict[str, Any]:
    ...
    if url.startswith("file:"):
        ...
        analytickit_version = plugin_json.get("analytickitVersion", None)
    ...
    if analytickit_version and not settings.MULTI_TENANCY:
        try:
            spec = SimpleSpec(analytickit_version.replace(" ", ""))
        except ValueError:
            raise ValidationError(f'Invalid analytickit semantic version requirement "{analytickit_version}"!')
        if not (Version(VERSION) in spec):
            raise ValidationError(
                f'Currently running analytickit version {VERSION} does not match this plugin\'s semantic version requirement "{analytickit_version}".'
            )
...
class Plugin(models.Model):
    ...
    organization: models.ForeignKey = models.ForeignKey(
        "analytickit.Organization", on_delete=models.CASCADE, related_name="plugins", related_query_name="plugin"
    )
    ...
    # DEPRECATED: this was used when syncing analytickit.json with the db on app start
...
class PluginStorage(models.Model):
    class Meta:
        constraints = [
            models.UniqueConstraint(fields=["plugin_config_id", "key"], name="analytickit_unique_plugin_storage_key")
        ]
