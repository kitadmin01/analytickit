...
from analytickit.constants import FILTER_TEST_ACCOUNTS, INSIGHT_FUNNELS
from analytickit.models import Action, ActionStep, Element
from analytickit.models.cohort import Cohort
from analytickit.models.filters import Filter
from analytickit.queries.funnels import ClickhouseFunnel, ClickhouseFunnelActors
from analytickit.queries.funnels.test.breakdown_cases import assert_funnel_results_equal, funnel_breakdown_test_factory
from analytickit.queries.funnels.test.conversion_time_cases import funnel_conversion_time_test_factory
...
class TestFunnelBreakdown(ClickhouseTestMixin,
                          funnel_breakdown_test_factory(ClickhouseFunnel, ClickhouseFunnelActors, _create_event,
                                                        _create_action, _create_person)):  # type: ignore
    maxDiff = None
    pass


class TestFunnelConversionTime(ClickhouseTestMixin,
                               funnel_conversion_time_test_factory(ClickhouseFunnel, ClickhouseFunnelActors,
                                                                   _create_event, _create_person)):  # type: ignore
    maxDiff = None
    pass


def funnel_test_factory(Funnel, event_factory, person_factory):
    @patch("analytickit.celery.update_cache_item_task.delay", update_cache_item)
    class TestGetFunnel(ClickhouseTestMixin, APIBaseTest):
        ...
        def test_funnel_person_prop(self):
                ...
                "events": [
                    {
                        ...
                        "type": "events",
                        "order": 0,
                        "properties": [{"key": "email", "value": "hello@analytickit.com", "type": "person"}, ],
                    },
                ],
                "actions": [
                    {"id": action_credit_card.pk, "type": "actions", "order": 1, },
            ...
            # events
            person_factory(
                distinct_ids=["with_property"], team_id=self.team.pk, properties={"email": "hello@analytickit.com"},
            )
        ...
        def test_funnel_filter_test_accounts(self):
            person_factory(distinct_ids=["person1"], team_id=self.team.pk, properties={"email": "test@analytickit.com"})
        ...
        def test_breakdown_values_is_set_on_the_query_with_fewer_than_two_entities(self):
            """
            failing test for https://sentry.io/organizations/analytickit/issues/2807609211/?project=1899813&referrer=slack
            """
