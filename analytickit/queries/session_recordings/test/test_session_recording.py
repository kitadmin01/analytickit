...
from analytickit.helpers.session_recording import ACTIVITY_THRESHOLD_SECONDS, DecompressedRecordingData, \
    RecordingSegment
from analytickit.models import Filter
from analytickit.models.team import Team
from analytickit.queries.session_recordings.session_recording import RecordingMetadata, SessionRecording
from analytickit.session_recordings.test.test_factory import create_chunked_snapshots, create_snapshot
    ...
    class TestSessionRecording(BaseTest):
        ...
        def test_get_metadata_for_non_existant_session_id(self):
            with freeze_time("2020-09-13T12:26:40.000Z"):
                ...
                recording = session_recording(team=self.team, session_recording_id="1",
                                              request=req).get_metadata()  # type: ignore
                self.assertEqual(
                    recording, None,
                )
