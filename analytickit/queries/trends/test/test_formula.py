...
from analytickit.constants import TRENDS_CUMULATIVE, TRENDS_PIE
...
from analytickit.test.base import APIBaseTest, ClickhouseTestMixin, _create_event, snapshot_clickhouse_queries
...
class TestFormula(ClickhouseTestMixin, APIBaseTest):
    ...
    def test_breakdown_mismatching_sizes(self):
        response = self._run(
            {"events": [{"id": "session start"}, {"id": "session end"}, ], "breakdown": "location",
             "formula": "A + B", }
        )
