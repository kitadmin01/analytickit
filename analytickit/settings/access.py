...
from analytickit.settings.utils import get_from_env, get_list, print_warning, str_to_bool
    ...
    if not TRUST_ALL_PROXIES and not TRUSTED_PROXIES:
        print_warning(
            (
                ...
                " https://analytickit.com/docs/configuring-analytickit/running-behind-proxy for details.",
            )
        )
...
if not DEBUG and not TEST and SECRET_KEY == DEFAULT_SECRET_KEY:
    print_warning(
        (
            ...
            "More information on https://analytickit.com/docs/self-host/configure/securing-analytickit",
        )
    )
