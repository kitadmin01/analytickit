...
from analytickit.email import EmailMessage, is_email_available
from analytickit.event_usage import report_first_ingestion_reminder_email_sent, \
    report_second_ingestion_reminder_email_sent
from analytickit.models import Organization, OrganizationInvite, OrganizationMembership, Plugin, PluginConfig, Team, \
    User
...
def send_invite(invite_id: str) -> None:
    ...
    message = EmailMessage(
        ...
        subject=f"{invite.created_by.first_name} invited you to join {invite.organization.name} on analytickit",
...
def send_member_join(invitee_uuid: str, organization_id: str) -> None:
    ...
    message = EmailMessage(
        campaign_key=campaign_key,
        subject=f"{invitee.first_name} joined you on analytickit",
...
def send_password_reset(user_id: int) -> None:
    ...
    message = EmailMessage(
        ...
        subject=f"Reset your analytickit password",
...
def send_canary_email(user_email: str) -> None:
    message = EmailMessage(
        ...
        subject="This is a test email of your analytickit instance",
...
def send_first_ingestion_reminder_emails() -> None:
    if is_email_available():
        ...
        for user in users_to_email:
            if analytickitanalytics.feature_enabled("re-engagement-emails", user.distinct_id):
                message = EmailMessage(
                    campaign_key=campaign_key,
                    subject="Get started: How to send events to analytickit",
                    reply_to="hey@analytickit.com",
    ...
    if is_email_available():
        ...
        for user in users_to_email:
            if analytickitanalytics.feature_enabled("re-engagement-emails", user.distinct_id):
                message = EmailMessage(
                    campaign_key=campaign_key,
                    subject="Your analytickit project is waiting for events",
                    reply_to="hey@analytickit.com",
