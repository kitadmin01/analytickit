...
from analytickit.models.event.util import (
...
from analytickit.models.person.util import count_duplicate_distinct_ids_for_team, count_total_persons_with_multiple_ids
from analytickit.models.plugin import PluginConfig
from analytickit.models.utils import namedtuplefetchall
from analytickit.utils import get_helm_info_env, get_instance_realm, get_machine_id, get_previous_week
...
def status_report(*, dry_run: bool = False) -> Dict[str, Any]:
    ...
    report["teams"] = {}
    report["table_sizes"] = {
        "analytickit_event": fetch_table_size("analytickit_event"),
        "analytickit_sessionrecordingevent": fetch_table_size("analytickit_sessionrecordingevent"),
    }
...
def capture_event(name: str, report: Dict[str, Any], dry_run: bool) -> None:
    if not dry_run:
        ...
        if "instance_usage_summary" in report:
            analytickitanalytics.group_identify("instance", settings.SITE_URL, fetch_instance_params(report))

        for user in User.objects.all():
            analytickitanalytics.capture(user.distinct_id, f"user {name}", {**report, "scope": "user"})
    else:
...
def fetch_instance_params(report: Dict[str, Any]) -> dict:
    return {
        ...
        "analytickit_version": report["analytickit_version"],
...
def fetch_event_counts_by_lib(params: Tuple[Any, ...]) -> dict:
    results = fetch_sql(
        ...
        FROM analytickit_event WHERE team_id = %s AND timestamp >= %s AND timestamp <= %s
        GROUP BY lib
        """,
        params,
    )
