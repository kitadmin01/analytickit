...
from analytickit.models import Organization, Plugin, Team
...
def factory_status_report(_create_event: Callable, _create_person: Callable):  # type: ignore
    class TestStatusReport(APIBaseTest):
        ...
        def test_status_report(self) -> None:
            ...
            self.assertEqual(report["analytickit_version"], VERSION)
            self.assertEqual(report["deployment"], "tests")
            self.assertLess(report["table_sizes"]["analytickit_event"], 10 ** 7)  # <10MB
            self.assertLess(report["table_sizes"]["analytickit_sessionrecordingevent"], 10 ** 7)  # <10MB
