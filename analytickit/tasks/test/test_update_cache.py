...
from analytickit.constants import ENTITY_ID, ENTITY_TYPE, INSIGHT_STICKINESS
from analytickit.decorators import CacheType
from analytickit.models import Dashboard, DashboardTile, Filter, Insight
...
from analytickit.models.filters.utils import get_filter
...
from analytickit.tasks.update_cache import synchronously_update_insight_cache, update_cache_item, update_cached_items
from analytickit.test.base import APIBaseTest
from analytickit.types import FilterType
from analytickit.utils import generate_cache_key, get_safe_cache
...
class TestSynchronousCacheUpdate(APIBaseTest):
    ...
    def redis_recency(self) -> Generator[List[int], None, None]:
        ...
        with patch("analytickit.tasks.update_cache.get_client") as mock_redis_get_client:
...
class TestUpdateCache(APIBaseTest):
    ...
    @patch("analytickit.tasks.update_cache.group.apply_async")
    @patch("analytickit.celery.update_cache_item_task.s")
    ...
    @freeze_time("2012-01-15")
    @patch("analytickit.queries.funnels.ClickhouseFunnelUnordered", create=True)
    @patch("analytickit.queries.funnels.ClickhouseFunnelStrict", create=True)
    @patch("analytickit.tasks.update_cache.ClickhouseFunnelTimeToConvert", create=True)
    @patch("analytickit.tasks.update_cache.ClickhouseFunnelTrends", create=True)
    @patch("analytickit.queries.funnels.ClickhouseFunnel", create=True)
    ...
    @patch("analytickit.tasks.update_cache._calculate_by_filter")
    ...
    @freeze_time("2021-08-25T22:09:14.252Z")
    @patch("analytickit.tasks.update_cache.insight_update_task_params")
    ...
    @patch("analytickit.tasks.update_cache.statsd.gauge")
    ...
    @patch("analytickit.tasks.update_cache._calculate_by_filter", return_value={"not": "None"})
    @patch("analytickit.tasks.update_cache.group.apply_async")
    @patch("analytickit.celery.update_cache_item_task.s")
    ...
    @patch("analytickit.tasks.update_cache.cache.set")
    @patch("analytickit.tasks.update_cache._calculate_by_filter")
    @patch("analytickit.tasks.update_cache.group.apply_async")
    @patch("analytickit.celery.update_cache_item_task.s")
    @patch("analytickit.tasks.update_cache.statsd.incr")
    ...
    @freeze_time("2021-08-25T22:09:14.252Z")
    @patch("analytickit.tasks.update_cache._calculate_by_filter", return_value={"not", "an empty result"})
    @patch("analytickit.tasks.update_cache.group.apply_async")
    @patch("analytickit.celery.update_cache_item_task.s")
    @patch("analytickit.tasks.update_cache.statsd.incr")
    ...
    @freeze_time("2021-08-25T22:09:14.252Z")
    @patch("analytickit.tasks.update_cache.cache.set")
    @patch("analytickit.tasks.update_cache._calculate_by_filter", return_value={"not": "empty result"})
    @patch("analytickit.tasks.update_cache.group.apply_async")
    @patch("analytickit.celery.update_cache_item_task.s")
...
class TestCacheTeamRecency(APIBaseTest):
    ...
    @patch("analytickit.tasks.update_cache.sync_execute")
    @patch("analytickit.tasks.update_cache.get_client")
