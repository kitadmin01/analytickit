...
from analytickit.client import ch_pool, sync_execute
from analytickit.models import Organization, Team, User
from analytickit.models.event.sql import DISTRIBUTED_EVENTS_TABLE_SQL, DROP_EVENTS_TABLE_SQL, EVENTS_TABLE_SQL
...
class TestMixin:
    ...
    CONFIG_EMAIL: Optional[str] = "user1@analytickit.com"
    ...
    def validate_basic_html(self, html_message, site_url, preheader=None):
        ...
        self.assertIn(f"{site_url}/static/analytickit-logo.png", html_message)  # type: ignore
...
class QueryMatchingTest:
    ...
    def assertQueryMatchesSnapshot(self, query, params=None, replace_all_numbers=False):
        ...
        query = re.sub(
            fr"""("organization_id"|"analytickit_organization"\."id") = '[^']+'::uuid""",
            r"""\1 = '00000000-0000-0000-0000-000000000000'::uuid""",
            query,
        )
        query = re.sub(
            fr"""("organization_id"|"analytickit_organization"\."id") IN \('[^']+'::uuid\)""",
        ...
        query = re.sub(
            fr"""("analytickit_tag"\."id") IN \(('[^']+'::uuid)+(, ('[^']+'::uuid)+)*\)""",
...
class ClickhouseTestMixin(QueryMatchingTest):
    ...
    # overrides the basetest in analytickit/test/base.py
    ...
    def capture_queries(self, query_prefixes: Union[str, Tuple[str, str]]):
        ...
        with patch("analytickit.client.ch_pool.get_client", wraps=get_client) as _:
            yield queries
