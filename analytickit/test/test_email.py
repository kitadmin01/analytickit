...
from analytickit.email import EmailMessage, _send_email
from analytickit.models import MessagingRecord, Organization, Person, Team, User
...
class TestEmail(BaseTest):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create(email="test@analytickit.com")
        self.user2 = User.objects.create(email="test2@analytickit.com")
        self.user_red_herring = User.objects.create(email="test+redherring@analytickit.com")
        ...
        MessagingRecord.objects.get_or_create(
            raw_email="test+redherring@analytickit.com",
    ...
    def test_cant_send_same_campaign_twice(self) -> None:
        with override_instance_config("EMAIL_HOST", "localhost"):
            ...
            record, _ = MessagingRecord.objects.get_or_create(raw_email="test0@analytickit.com",
            ...
            with self.settings(CELERY_TASK_ALWAYS_EAGER=True):
                _send_email(
                    campaign_key="campaign_1",
                    to=[{"raw_email": "test0@analytickit.com",
                         "recipient": "Test analytickit <test0@analytickit.com>"}],
                    subject="Test email",
                    headers={},
                )
    ...
    def test_applies_default_utm_tags(self) -> None:
        with override_instance_config("EMAIL_HOST", "localhost"):
            ...
            assert (
                    f"https://analytickit.com/questions?utm_source=analytickit&amp;utm_medium=email&amp;utm_campaign={template}"
