...
from analytickit.models import Organization, OrganizationInvite, Plugin
from analytickit.plugins.test.mock import mocked_plugin_requests_get
from analytickit.plugins.test.plugin_archives import HELLO_WORLD_PLUGIN_GITHUB_ZIP
...
class TestOrganization(BaseTest):
    ...
    def test_plugins_are_preinstalled_on_self_hosted(self, mock_get):
        with self.settings(
                MULTI_TENANCY=False, PLUGINS_PREINSTALLED_URLS=["https://github.com/analytickit/helloworldplugin/"]
        ):
        ...
        mock_get.assert_called_with(
            f"https://github.com/analytickit/helloworldplugin/archive/{HELLO_WORLD_PLUGIN_GITHUB_ZIP[0]}.zip",
            headers={}
        )
    ...
    def test_plugins_are_not_preinstalled_on_cloud(self, mock_get):
        with self.settings(
                MULTI_TENANCY=True, PLUGINS_PREINSTALLED_URLS=["https://github.com/analytickit/helloworldplugin/"]
        ):
            new_org, _, _ = Organization.objects.bootstrap(
                self.user, plugins_access_level=Organization.PluginsAccessLevel.INSTALL
            )
