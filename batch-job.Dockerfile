...
# Added aws and pg_dump libs for db backup. Couldn't add clickhouse-client for clickhouse backup
#
...
RUN apk --update --no-cache add \
    postgresql-client \
    # For ClickHouse and AWS CLI, you might need to find alternative ways to install, as they might not be available in the default Alpine repositories.
    # For AWS CLI, you can install using pip
    py-pip \
    && pip install awscli \
