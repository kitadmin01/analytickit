...
export type AnalyticKitStorage = {
...
export const cookieStore: AnalyticKitStorage = {
  getItem(key) {
    try {
      const nameEQ = key + '='
...
const createStorageLike = (store: any): AnalyticKitStorage => {
  return {
    getItem(key) {
...
const checkStoreIsSupported = (storage: AnalyticKitStorage, key = '__mplssupport__'): boolean => {
  if (!window) {
    return false
  }
  try {
...
const createMemoryStorage = (): AnalyticKitStorage => {
  const _cache: { [key: string]: any | undefined } = {}

  const store: AnalyticKitStorage = {
    getItem(key) {
      return _cache[key]
    },
...
export const getStorage = (type: AnalyticKitOptions['persistence']): AnalyticKitStorage => {
  switch (type) {
    case 'cookie':
