...
- fix: Testcafe using Analytickit cloud (#428)
...
- pass toolbar to analytickit (#327)
...
- Revert "Add analytickit.people.increment" (#320)
...
- Add analytickit.people.increment (#254)
...
- Revert "Speculative logging for AnalyticKit/analytickit#4816 (#293)" (#302)
- Filter out _ngcontent attributes in autocapture (#298)
- corrects a test where assertion and setup didn't match test name (#299)

## 1.13.15 - 2021-09-29

- Speculative logging for AnalyticKit/analytickit#4816 (#293)
...
- Add analytickit.debug() to types, remove bad docstring (#201)
...
- Event names must be strings in `analytickit.capture` (#171)
...
- Using gzip-based compression over lzstring using the fflate library: [fflate](https://github.com/101arrowz/fflate). This reduces the amount of data transferred, and makes analytickit servers respond faster (requires analytickit 1.19.0). https://github.com/AnalyticKit/analytickit/issues/2560
- Support last touch $referrer and $referring_domain user properties https://github.com/AnalyticKit/analytickit-js/pull/139
- Publish a ES dist file https://github.com/AnalyticKit/analytickit-js/pull/157
- Publish a react integration for feature flags https://github.com/AnalyticKit/analytickit-js/pull/154
...
## 1.7.1 - 2020-11-27
- Force session recording to use lz64 compression (https://github.com/AnalyticKit/analytickit-js/pull/134)
- Bundle module.js in es5 (https://github.com/AnalyticKit/analytickit-js/pull/132)

## 1.7.0 - 2020-11-26
- Send session recording events to analytickit in (short) batches, separate from rest of events to make sure we drop fewer events (#126)
- Send session recording events to a separate endpoint for newer versions of analytickit (#118)
- Send correct LIB_VERSION to analytickit with captures (#119)
...
- Deprecate calling analytickit.capture with a callback (#129)
- Attempted to re-add support for including analytickit-js in server-side rendering. (#131)
- Bugfix: Don't truncate session recording data (#121)
- Bugfix: Kill `analytickit.capture_links()` and `analytickit.capture_forms()`. They were broken since initial release - you can use autocapture instead. (#128)

## 1.6.0 - 2020-11-05
- Allow updating user properties when calling `analytickit.identify('identity, { some: 'value' })` (#105)
- Allow disabling $feature_flag_called event: `analytickit.isFeatureEnabled('flag', { send_event: false }) (#100)
- Make cookieless analytics possible by passing `persistence: 'memory'` to analytickit.init (#82)
- Avoid sending $pageleave events when `capture_pageview: false` passed to analytickit.init (#109)
...
- Improve typing of analytickit-js (#103)

## 1.5.1 - 2020-10-22
- Improve typing of analytickit.js #97 (thanks @stonesthatwhisper)
...
- Add option `.reloadFeatureFlags()`. Call it to trigger a reload of feature flags. (See [#71](https://github.com/AnalyticKit/analytickit-js/pull/71))
- Add config option `sanitize_properties` that accepts a function which sanitizes parameters of events (See [#75](https://github.com/AnalyticKit/analytickit-js/issues/75))
...
## 1.3.1 - 2020-07-13
- Support loading the toolbar with a `__analytickit` has param (was: `state`) and `ph_authorize` action.

## 1.3.0 - 2020-07-03
- Add TypeScript defintions

## 1.2.4 - 2020-07-01
- Add support for feature flags (`analytickit.isFeatureEnabled('keyword')`)
...
## 1.1.2 - 2020-06-04
- Fix another error when using a new analytickit-js version with an old analytickit version

## 1.1.1 - 2020-06-04
- Show a error if calling `analytickit.identify` with `null` user (#34 by @rushabhnagda11)

## 1.1.0 - 2020-06-04
- Support loading new AnalyticKit toolbar
