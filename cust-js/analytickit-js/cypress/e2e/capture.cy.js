...
describe('Event capture', () => {
    ...
    const start = ({ waitForDecide = true } = {}) => {
        ...
        cy.analytickitInit(given.options)
        if (waitForDecide) {
            cy.wait('@decide')
        }
    }
    ...
    describe('group analytics', () => {
        given('options', () => ({
            loaded: (analytickit) => {
                analytickit.group('company', 'id:5')
            },
        }))
    ...
    describe('subsequent decide calls', () => {
        given('options', () => ({
            loaded: (analytickit) => {
                analytickit.identify('new-id')
                analytickit.group('company', 'id:5', { id: 5, company_name: 'Awesome Inc' })
                analytickit.group('playlist', 'id:77', { length: 8 })
            },
        }))
        ...
        it('does a single decide call on following changes', () => {
            ...
            cy.analytickit().invoke('group', 'company', 'id:6')
            cy.analytickit().invoke('group', 'playlist', 'id:77')
            cy.analytickit().invoke('group', 'anothergroup', 'id:99')
