import { AnalyticKitPersistence } from '../analytickit-persistence'

given('lib', () => new AnalyticKitPersistence({ name: 'bla', persistence: 'cookie' }))
...
describe('persistence', () => {
    ...
    it('should migrate data from cookies to localStorage', () => {
        let lib = new AnalyticKitPersistence({ name: 'bla', persistence: 'cookie' })
        lib.register_once({ distinct_id: 'testy', test_prop: 'test_value' })
        expect(document.cookie).toEqual(
            'ph__analytickit=%7B%22distinct_id%22%3A%22testy%22%2C%22test_prop%22%3A%22test_value%22%7D'
        )
        let lib2 = new AnalyticKitPersistence({ name: 'bla', persistence: 'localStorage+cookie' })
        expect(document.cookie).toEqual('ph__analytickit=%7B%22distinct_id%22%3A%22testy%22%7D')
        lib2.register({ test_prop2: 'test_val', distinct_id: 'test2' })
        expect(document.cookie).toEqual('ph__analytickit=%7B%22distinct_id%22%3A%22test2%22%7D')
        expect(lib2.props).toEqual({ distinct_id: 'test2', test_prop: 'test_value', test_prop2: 'test_val' })
        lib2.remove('ph__analytickit')
        expect(localStorage.getItem('ph__analytickit')).toEqual(null)
        expect(document.cookie).toEqual('')
    })

    it(`should register once LS`, () => {
        let lib = new AnalyticKitPersistence({ name: 'test', persistence: 'localStorage+cookie' })
        lib.register_once({ distinct_id: 'hi', test_prop: 'test_val' })

        let lib2 = new AnalyticKitPersistence({ name: 'test', persistence: 'localStorage+cookie' })
    ...
    forPersistenceTypes(function (persistenceType) {
        it(`should register once`, () => {
            let lib = new AnalyticKitPersistence({ name: 'test', persistence: persistenceType })
            lib.register_once({ distinct_id: 'hi', test_prop: 'test_val' })

            let lib2 = new AnalyticKitPersistence({ name: 'test', persistence: persistenceType })
            expect(lib2.props).toEqual({ distinct_id: 'hi', test_prop: 'test_val' })
            lib.clear()
            lib2.clear()
        })
