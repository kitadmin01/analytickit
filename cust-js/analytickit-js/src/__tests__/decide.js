...
describe('Decide', () => {
    given('decide', () => new Decide(given.analytickit))
    given('analytickit', () => ({
    ...
    describe('constructor', () => {
        ...
        it('should call instance._send_request on constructor', () => {
            ...
            expect(given.analytickit._send_request).toHaveBeenCalledWith(
    ...
    describe('parseDecideResponse', () => {
        ...
        it('properly parses decide response', () => {
            ...
            expect(given.analytickit.sessionRecording.afterDecideResponse).toHaveBeenCalledWith(given.decideResponse)
            expect(given.analytickit.toolbar.afterDecideResponse).toHaveBeenCalledWith(given.decideResponse)
            expect(given.analytickit.featureFlags.receivedFeatureFlags).toHaveBeenCalledWith(given.decideResponse)
            expect(autocapture.afterDecideResponse).toHaveBeenCalledWith(given.decideResponse, given.analytickit)
        })
        ...
        it('enables compression from decide response', () => {
            ...
            expect(given.analytickit.compression['gzip']).toBe(true)
            expect(given.analytickit.compression['lz64']).toBe(true)
        })
        ...
        it('Make sure receivedFeatureFlags is not called if the decide response fails', () => {
            ...
            expect(given.analytickit.featureFlags.receivedFeatureFlags).not.toHaveBeenCalled()
            expect(console.error).toHaveBeenCalledWith('Failed to fetch feature flags from AnalyticKit.')
        })
    })
})
