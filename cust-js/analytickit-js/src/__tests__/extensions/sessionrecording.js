...
import { AnalyticKitPersistence, SESSION_RECORDING_ENABLED_SERVER_SIDE } from '../../analytickit-persistence'
...
describe('SessionRecording', () => {
    ...
    given('sessionRecording', () => new SessionRecording(given.analytickit))
    ...
    describe('afterDecideResponse()', () => {
        ...
        it('stores true in persistence if recording is enabled from the server', () => {
            given.subject()
            expect(given.analytickit.persistence.register).toHaveBeenCalledWith({
    ...
    describe('recording', () => {
        ...
        it('records events emitted before and after starting recording', () => {
            ...
            _emit({ event: 1 })
            expect(given.analytickit.capture).not.toHaveBeenCalled()

            given.sessionRecording.afterDecideResponse({ endpoint: '/s/' })
            _emit({ event: 2 })

            expect(given.analytickit.capture).toHaveBeenCalledTimes(2)
            expect(given.analytickit.capture).toHaveBeenCalledWith(
                '$snapshot',
                {
        ...
        describe('session and window ids', () => {
            ...
            it('it calls checkAndGetSessionAndWindowId with readOnly as true if it not a user interaction', () => {
                ...
                expect(given.analytickit.sessionManager.checkAndGetSessionAndWindowId).toHaveBeenCalledWith(
                    true,
                    undefined
                )
            })
        ...
        describe('with a real session id manager', () => {
            ...
            beforeEach(() => {
                given(
                    'sessionManager',
                    () => new SessionIdManager(given.config, new AnalyticKitPersistence(given.config))
                )
                given.sessionRecording.startRecordingIfEnabled()
                given.sessionRecording.startCaptureAndTrySendingQueuedSnapshots()
            })
