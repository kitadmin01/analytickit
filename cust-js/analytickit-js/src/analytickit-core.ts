...
import { AnalyticKitFeatureFlags } from './analytickit-featureflags'
import { ALIAS_ID_KEY, PEOPLE_DISTINCT_ID_KEY, AnalyticKitPersistence } from './analytickit-persistence'
...
// TODO: the type of this is very loose. Sometimes it's also AnalyticKitLib itself
let analytickit_master: Record<string, AnalyticKit> & {
    init: (token: string, config: Partial<AnalyticKitConfig>, name: string) => void
}
...
const PRIMARY_INSTANCE_NAME = 'analytickit'
...
const defaultConfig = (): AnalyticKitConfig => ({
    ...
    name: 'analytickit',
    callback_fn: 'analytickit._jsc',
})

/**
 * create_mplib(token:string, config:object, name:string)
 *
 * This function is used by the init method of AnalyticKitLib objects
 * as well as the main initializer at the end of the JSLib (that
 * initializes document.analytickit as well as any additional instances
 * declared before this file has loaded).
 */
const create_mplib = function (token: string, config?: Partial<AnalyticKitConfig>, name?: string): AnalyticKit {
    let instance: AnalyticKit
    const target =
        name === PRIMARY_INSTANCE_NAME || !analytickit_master
            ? analytickit_master
            : name
            ? analytickit_master[name]
            : undefined
...
/**
 * AnalyticKit Library Object
 * @constructor
 */
export class AnalyticKit {
    __loaded: boolean
    config: AnalyticKitConfig

    persistence: AnalyticKitPersistence
    sessionManager: SessionIdManager
    people: AnalyticKitPeople
    featureFlags: AnalyticKitFeatureFlags
    feature_flags: AnalyticKitFeatureFlags
    toolbar: Toolbar
    ...
    constructor() {
        ...
        this.people = new AnalyticKitPeople(this)
        this.featureFlags = new AnalyticKitFeatureFlags(this)
    ...
    /**
     * This function initializes a new instance of the AnalyticKit capturing object.
     * All new instances are added to the main analytickit object as sub properties (such as
     * analytickit.library_name) and also returned by this function. To define a
     * second instance on the page, you would call:
     *
     *     analytickit.init('new token', { your: 'config' }, 'library_name');
     *
     * and use it like so:
     *
     *     analytickit.library_name.capture(...);
     *
     * @param {String} token   Your AnalyticKit API token
     * @param {Object} [config]  A dictionary of config options to override. <a href="https://github.com/analytickit/analytickit-js/blob/6e0e873/src/analytickit-core.js#L57-L91">See a list of default config options</a>.
     * @param {String} [name]    The name for the new analytickit instance that you want created
     */
    init(token: string, config?: Partial<AnalyticKitConfig>, name?: string): AnalyticKit | void {
        ...
        if (name === PRIMARY_INSTANCE_NAME) {
            console.error(
                'You must initialize the main analytickit object right after you include the AnalyticKit js snippet'
            )
            return
        }

        const instance: AnalyticKit = create_mplib(token, config, name)
        analytickit_master[name] = instance
        instance._loaded()

        return instance
    }

    // analytickit._init(token:string, config:object, name:string)
    //
    // This function sets up the current instance of the analytickit
    // library.  The difference between this method and the init(...)
    // method is this one initializes the actual instance, whereas the
    // init(...) method sets up a new library and calls _init on it.
    //
    _init(token: string, config: Partial<AnalyticKitConfig> = {}, name?: string): void {
        this.__loaded = true
        this.config = {} as AnalyticKitConfig // will be set right below
        ...
        this.persistence = new AnalyticKitPersistence(this.config)
    ...
    /**
     * _execute_array() deals with processing any analytickit function
     * calls that were called before the AnalyticKit library were loaded
     * (and are thus stored in an array so they can be called later)
     *
     * Note: we fire off all the analytickit function calls && user defined
     * functions BEFORE we fire off analytickit capturing calls. This is so
    ...
    _execute_array(array: SnippetArrayItem[]): void {
        ...
        _eachArray(array, (item) => {
            if (item) {
                fn_name = item[0]
                if (_isArray(fn_name)) {
                    capturing_calls.push(item) // chained call e.g. analytickit.get_group().set()
     ...
     * frequently used AnalyticKit function.
     *
     * ### Usage:
     *
     *     // capture an event named 'Registered'
     *     analytickit.capture('Registered', {'Gender': 'Male', 'Age': 21});
     *
     *     // capture an event using navigator.sendBeacon
     *     analytickit.capture('Left page', {'duration_seconds': 35}, {transport: 'sendBeacon'});
     *
    ...
    capture(
        ...
        if (_isUndefined(event_name) || typeof event_name !== 'string') {
            console.error('No event name provided to analytickit.capture')
            return
        }
        ...
        if (this.get_config('debug')) {
            logger.log('AnalyticKit.js send', data)
        }
        const jsonData = JSON.stringify(data)
     ...
     *     analytickit.register({'Gender': 'Female'});
     *
     ...
     * ### Usage:
     *
     *     if(analytickit.getFeatureFlag('beta-feature') === 'some-value') { // do something }
     *
     * @param {Object|String} prop Key of the feature flag.
     * @param {Object|String} options (optional) If {send_event: false}, we won't send an $feature_flag_call event to AnalyticKit.
     */
     ...
     * ### Usage:
     *
     *     if(analytickit.isFeatureEnabled('beta-feature')) { // do something }
     *
     * @param {Object|String} prop Key of the feature flag.
     * @param {Object|String} options (optional) If {send_event: false}, we won't send an $feature_flag_call event to AnalyticKit.
     */
     ...
     * ### Usage:
     *
     *     analytickit.onFeatureFlags(function(featureFlags) { // do something })
     *
    ...
    /**
     * Identify a user with a unique ID instead of a AnalyticKit
     * randomly generated distinct_id. If the method is never called,
     * then unique visitors will be identified by a UUID generated
     * the first time they visit the site.
     *
     * If user properties are passed, they are also sent to analytickit.
     *
     * ### Usage:
     *
     *      analytickit.identify('[user unique id]')
     *      analytickit.identify('[user unique id]', { email: 'john@example.com' })
     *      analytickit.identify('[user unique id]', {}, { referral_code: '12345' })
     *
     * ### Notes:
     *
     * You can call this function to overwrite a previously set
     * unique ID for the current user. AnalyticKit cannot translate
     * between IDs at this time, so when you change a user's ID
     * they will appear to be a new user.
     *
     * When used alone, analytickit.identify will change the user's
     * distinct_id to the unique ID provided. When used in tandem
     * with analytickit.alias, it will allow you to identify based on
    ...
    identify(new_distinct_id?: string, userPropertiesToSet?: Properties, userPropertiesToSetOnce?: Properties): void {
        ...
        if (!new_distinct_id) {
            console.error('Unique user id has not been set in analytickit.identify')
            return
        }
    ...
    group(groupType: string, groupKey: string, groupPropertiesToSet?: Properties): void {
        if (!groupType || !groupKey) {
            console.error('analytickit.group requires a group type and group key')
            return
        }
     ...
     * ### Notes:
     *
     * get_distinct_id() can only be called after the AnalyticKit library has finished loading.
     * init() has a loaded function available to handle this automatically. For example:
     *
     *     // set distinct_id after the analytickit library has loaded
     *     analytickit.init('YOUR PROJECT TOKEN', {
     *         loaded: function(analytickit) {
     *             distinct_id = analytickit.get_distinct_id();
     *         }
     *     });
     */
    ...
    /**
     * Create an alias, which AnalyticKit will use to link two distinct_ids going forward (not retroactively).
     * Multiple aliases can map to the same original ID, but not vice-versa. Aliases can also be chained - the
     * following is a valid scenario:
     *
     *     analytickit.alias('new_id', 'existing_id');
     *     ...
     *     analytickit.alias('newer_id', 'new_id');
     *
    ...
    alias(alias: string, original?: string): CaptureResult | void | number {
        ...
        // analytickit.people.identify() call made for this user. It is VERY BAD to make an alias with
    ...
    /**
     * Update the configuration of a analytickit library instance.
     *
     ...
     *       // or any way to know the result of the request. AnalyticKit
     ...
     *       debug: false
     *
     *       // if this is true, the analytickit cookie or localStorage entry
     *       // will be deleted, and no user persistence will take place
     *       disable_persistence: false
     *
     *       // if this is true, AnalyticKit will automatically determine
     *       // City, Region and Country data using the IP address of
     *       //the client
     *       ip: true
     *
     *       // opt users out of capturing by this AnalyticKit instance by default
     *       opt_out_capturing_by_default: false
     *
     *       // opt users out of browser data storage by this AnalyticKit instance by default
     *       opt_out_persistence_by_default: false
     *
     ...
     *       // analytickit cookie value with the same persistence_name
     ...
     *       property_blacklist: []
     *
     *       // if this is true, analytickit cookies will be marked as
     ...
     *       inapp_link_new_window: false
     *
     *      // a set of rrweb config options that AnalyticKit users can configure
     *      // see https://github.com/rrweb-io/rrweb/blob/master/guide.md
     *      session_recording: {
     *         blockClass: 'ph-no-capture',
     *         blockSelector: null,
    ...
    set_config(config: Partial<AnalyticKitConfig>): void {
     ...
     */
    get_config<K extends keyof AnalyticKitConfig>(prop_name: K): AnalyticKitConfig[K] {
     ...
     * ### Notes:
     *
     * get_property() can only be called after the AnalyticKit library has finished loading.
     * init() has a loaded function available to handle this automatically. For example:
     *
     *     // grab value for 'user_id' after the analytickit library has loaded
     *     analytickit.init('YOUR PROJECT TOKEN', {
     *         loaded: function(analytickit) {
     *             user_id = analytickit.get_property('user_id');
     *         }
     *     });
     *
    ...
    /**
     * Opt the user in to data capturing and cookies/localstorage for this AnalyticKit instance
     *
     * ### Usage
     *
     *     // opt user in
     *     analytickit.opt_in_capturing();
     *
     ...
     * @param {function} [options.capture] Function used for capturing a AnalyticKit event to record the opt-in action (default is this AnalyticKit instance's capture method)
     ...
     * @param {Number} [options.cookie_expiration] Number of days until the opt-in cookie expires (overrides value specified in this AnalyticKit instance's config)
     * @param {boolean} [options.cross_subdomain_cookie] Whether the opt-in cookie is set as cross-subdomain or not (overrides value specified in this AnalyticKit instance's config)
     * @param {boolean} [options.secure_cookie] Whether the opt-in cookie is set as secure or not (overrides value specified in this AnalyticKit instance's config)
     */
    ...
    /**
     * Opt the user out of data capturing and cookies/localstorage for this AnalyticKit instance
     *
     * ### Usage
     *
     *     // opt user out
     *     analytickit.opt_out_capturing();
     *
     *     // opt user out with different cookie configuration from AnalyticKit instance
     *     analytickit.opt_out_capturing({
     *         cookie_expiration: 30,
     *         secure_cookie: true
     *     });
    ...
    /**
     * Check whether the user has opted in to data capturing and cookies/localstorage for this AnalyticKit instance
     *
     * ### Usage
     *
     *     const has_opted_in = analytickit.has_opted_in_capturing();
    ...
    /**
     * Check whether the user has opted out of data capturing and cookies/localstorage for this AnalyticKit instance
     *
     * ### Usage
     *
     *     const has_opted_out = analytickit.has_opted_out_capturing();
    ...
    /**
     * Clear the user's opt in/out status of data capturing and cookies/localstorage for this AnalyticKit instance
     *
     * ### Usage
     *
     *     // clear user's opt-in/out status
     *     analytickit.clear_opt_in_out_capturing();
     *
    ...
    debug(debug?: boolean): void {
        if (debug === false) {
            ...
            window.console.log(
                "You're now in debug mode. All calls to AnalyticKit will be logged in your console.\nYou can disable this with `analytickit.debug(false)`."
            )
...
_safewrap_class(AnalyticKit, ['identify'])

const instances: Record<string, AnalyticKit> = {}
const extend_mp = function () {
    // add all the sub analytickit instances
...
const override_ph_init_func = function () {
    ...
    // user initializes the analytickit library after the script loads & runs
    analytickit_master['init'] = function (token?: string, config?: Partial<AnalyticKitConfig>, name?: string) {
        if (name) {
            // initialize a sub library
            if (!analytickit_master[name]) {
                analytickit_master[name] = instances[name] = create_mplib(token || '', config || {}, name)
                analytickit_master[name]._loaded()
            }
            return analytickit_master[name]
        } else {
            let instance: AnalyticKit = analytickit_master as any as AnalyticKit

            if (instances[PRIMARY_INSTANCE_NAME]) {
                // main analytickit lib already initialized
                instance = instances[PRIMARY_INSTANCE_NAME]
            } else if (token) {
                // intialize the main analytickit lib
                instance = create_mplib(token, config || {}, PRIMARY_INSTANCE_NAME)
                instance._loaded()
                instances[PRIMARY_INSTANCE_NAME] = instance
            }

            ;(analytickit_master as any) = instance
            if (init_type === InitType.INIT_SNIPPET) {
                ;(window as any)[PRIMARY_INSTANCE_NAME] = analytickit_master
            }
            extend_mp()
            return instance
        }
...
const add_dom_loaded_handler = function () {
    ...
    function dom_loaded_handler() {
        ...
        _each(instances, function (inst: AnalyticKit) {
            inst._dom_loaded()
        })
    }
...
export function init_from_snippet(): void {
    ...
    if (_isUndefined((window as any).analytickit)) {
        ;(window as any).analytickit = []
    }
    analytickit_master = (window as any).analytickit

    if (analytickit_master['__loaded'] || (analytickit_master['config'] && analytickit_master['persistence'])) {
        // lib has already been loaded at least once; we don't want to override the global object this time so bomb early
        console.error('AnalyticKit library has already been downloaded at least once.')
        return
    }

    // Load instances of the AnalyticKit Library
    _each(analytickit_master['_i'], function (item: [token: string, config: Partial<AnalyticKitConfig>, name: string]) {
    ...
    ;(analytickit_master['init'] as any)()

    // Fire loaded events after updating the window's analytickit object
...
export function init_as_module(): AnalyticKit {
    init_type = InitType.INIT_MODULE
    ;(analytickit_master as any) = new AnalyticKit()

    override_ph_init_func()
    ;(analytickit_master['init'] as any)()
    add_dom_loaded_handler()

    return analytickit_master as any
}
