...
import { AnalyticKitConfig, Properties, RequestCallback } from './types'
...
/**
 * AnalyticKit People Object
 * @constructor
 */
class AnalyticKitPeople {
    _analytickit: AnalyticKit

    set: (prop: string | Properties, to?: string, callback?: RequestCallback) => void
    set_once: (prop: string | Properties, to?: string, callback?: RequestCallback) => void

    constructor(analytickit: AnalyticKit) {
        this._analytickit = analytickit
         ...
         * ### Usage:
         *
         *     analytickit.people.set('gender', 'm');
         *
         ...
         */
        this.set = addOptOutCheck(analytickit, (prop: string | Properties, to?: string, callback?: RequestCallback) => {
            ...
            if (this._get_config('save_referrer')) {
                this._analytickit.persistence.update_referrer_info(document.referrer)
            }
            ...
            data[SET_ACTION] = _extend(
                ...
                this._analytickit.persistence.get_referrer_info(),
                data[SET_ACTION]
            )
         ...
         * ### Usage:
         *
         *     analytickit.people.set_once('First Login Date', new Date());
         *
         *     // or set multiple properties at once
         *     analytickit.people.set_once({
    ...
    toString(): string {
        return this._analytickit.toString() + '.people'
    }

    _send_request(data: Properties, callback?: RequestCallback): Properties {
        data['$token'] = this._get_config('token')
        data['$distinct_id'] = this._analytickit.get_distinct_id()
        const device_id = this._analytickit.get_property('$device_id')
        const user_id = this._analytickit.get_property('$user_id')
        const had_persisted_distinct_id = this._analytickit.get_property('$had_persisted_distinct_id')
        if (device_id) {
        ...
        this._analytickit._send_request(
            this._get_config('api_host') + '/engage/',
            { data: encoded_data },
            {},
            this._analytickit._prepare_callback(callback, truncated_data) as RequestCallback
        )

        return truncated_data
    }

    _get_config<K extends keyof AnalyticKitConfig>(conf_var: K): AnalyticKitConfig[K] {
        return this._analytickit.get_config(conf_var)
    }
    ...
    // Internal methods for analytickit.people API.
