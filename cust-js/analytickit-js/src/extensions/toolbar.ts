...
export class Toolbar {
    ...
    maybeLoadEditor(
        ...
        try {
            ...
            const stateHash = _getHashParam(location.hash, '__analytickit') || _getHashParam(location.hash, 'state')
            ...
            if (parseFromUrl) {
                ...
                if (editorParams && Object.keys(editorParams).length > 0) {
                    localStorage.setItem('_analyticKitEditorParams', JSON.stringify(editorParams))
                ...
                editorParams = JSON.parse(localStorage.getItem('_analyticKitEditorParams') || '{}')
    ...
    _loadEditor(editorParams: EditorParams) {
        if (!(window as any)['_analyticKitToolbarLoaded']) {
            // only load the codeless event editor once, even if there are multiple instances of AnalyticKitLib
            ;(window as any)['_analyticKitToolbarLoaded'] = true
            ...
            _register_event(window, 'turbolinks:load', () => {
                ;(window as any)['_analyticKitToolbarLoaded'] = false
                this._loadEditor(editorParams)
            })
            return true
        }
