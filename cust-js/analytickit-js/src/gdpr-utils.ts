 ...
 *
 * This set of utilities is intended to enable opt in/out functionality in the AnalyticKit JS SDK.
...
/**
 * A function used to capture a AnalyticKit event (e.g. AnalyticKitLib.capture)
 ...
 * @param {string} token - AnalyticKit project capturing token
 * @param {Object} [options]
 * @param {captureFunction} [options.capture] - function used for capturing a AnalyticKit event to record the opt-in action
...
export function userOptedOut(analytickit: AnalyticKit, silenceErrors: boolean | undefined) {
    let optedOut = false

    try {
        const token = analytickit.get_config('token')
        const respectDnt = analytickit.get_config('respect_dnt')
        const persistenceType = analytickit.get_config('opt_out_capturing_persistence_type')
        const persistencePrefix = analytickit.get_config('opt_out_capturing_cookie_prefix') || undefined
        const win = analytickit.get_config('window' as any) as Window | undefined // used to override window during browser tests
 ...
 * @param {AnalyticKit} analytickit - the analytickit instance
...
export function addOptOutCheck<M extends (...args: any[]) => any = (...args: any[]) => any>(
    ...
    return function (...args) {
        const optedOut = userOptedOut(analytickit, silenceErrors)
