...
const logger = {
    ...
    log: function (...args: any[]) {
        if (Config.DEBUG && !_isUndefined(window.console) && window.console) {
            // Don't log AnalyticKit debug messages in rrweb
            const log =
    ...
    error: function (..._args: any[]) {
        if (Config.DEBUG && !_isUndefined(window.console) && window.console) {
            const args = ['AnalyticKit error:', ..._args]
            // Don't log AnalyticKit debug messages in rrweb
            const error =
...
export const _safewrap = function <F extends (...args: any[]) => any = (...args: any[]) => any>(f: F): F {
    return function (...args) {
        try {
            ...
            logger.critical('Implementation error. Please turn on debug and contact support@analytickit.com.')
            if (Config.DEBUG) {
                logger.critical(e)
            }
        }
