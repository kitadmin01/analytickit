...
import { retryUntilResults, queryAPI, initAnalytickit, captureLogger, staticFilesMock } from './helpers'

fixture('analytickit.js capture')
    ...
    .afterEach(async () => {
        ...
        // console.debug('Requests to analytickit:', JSON.stringify(captureLogger.requests, null, 2))
    })

test('Custom events work and are accessible via /api/event', async (t) => {
    const testSessionId = await initAnalytickit()
    await t
        .wait(5000)
...
test('Config options change autocapture behavior accordingly', async (t) => {
    const testSessionId = await initAnalytickit({ mask_all_text: true, mask_all_element_attributes: true })
