...
class FeatureFlagsPoller {
  ...
  variantLookupTable(flag: AnalyticKitFeatureFlag): { valueMin: number; valueMax: number; key: string }[] {
    ...
    try {
      ...
      if (res && res.statusCode === 401) {
        throw new ClientError(
          `Your personalApiKey is invalid. Are you sure you're not using your Project API key? More information: https://analytickit.com/docs/api/overview`
        )
      }
  ...
  async _requestFeatureFlagDefinitions(): Promise<ResponseData> {
    ...
    const headers = {
      ...
      'user-agent': `analytickit-node/${version}`,
    }
