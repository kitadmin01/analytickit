// import AnalyticKit from '../'
// Uncomment below line while developing to not compile code everytime
import { AnalyticKitGlobal as AnalyticKit } from '../src/analytickit-node'
...
describe('local evaluation', () => {
  let analytickit: AnalyticKit

  jest.useFakeTimers()

  afterEach(async () => {
    // ensure clean shutdown & no test interdependencies
    await analytickit.shutdownAsync()
  })
  ...
  it('evaluates person properties', async () => {
    ...
    analytickit = new AnalyticKit('TEST_API_KEY', {
      host: 'http://example.com',
      personalApiKey: 'TEST_PERSONAL_API_KEY',
    })

    expect(
      await analytickit.getFeatureFlag('person-flag', 'some-distinct-id', { personProperties: { region: 'USA' } })
    ).toEqual(true)
    expect(
      await analytickit.getFeatureFlag('person-flag', 'some-distinct-id', { personProperties: { region: 'Canada' } })
    ).toEqual(false)
  ...
  it('evaluates group properties', async () => {
    ...
    expect(
      await analytickit.getFeatureFlag('group-flag', 'some-distinct-id', {
        groupProperties: { company: { name: 'Project Name 1' } },
      })
    ).toEqual(false)
    expect(
      await analytickit.getFeatureFlag('group-flag', 'some-distinct-2', {
  ...
  it('evaluates flag with complex definition', async () => {
    ...
    expect(
      await analytickit.getFeatureFlag('complex-flag', 'some-distinct-id', {
    ...
    expect(
      await analytickit.getFeatureFlag('complex-flag', 'some-distinct-id_within_rollout?', {
    ...
    expect(
      await analytickit.getFeatureFlag('complex-flag', 'some-distinct-id_outside_rollout?', {
        personProperties: { region: 'USA', email: 'a@b.com' },
      })
    ).toEqual('decide-fallback-value')
    ...
    // # same as above
    expect(
      await analytickit.getFeatureFlag('complex-flag', 'some-distinct-id', { personProperties: { doesnt_matter: '1' } })
    ).toEqual('decide-fallback-value')
    expect(mockedUndici.fetch).toHaveBeenCalledWith(
      'http://example.com/decide/?v=2',
      expect.objectContaining({
    ...
    expect(
      await analytickit.getFeatureFlag('complex-flag', 'some-distinct-id', { personProperties: { region: 'USA' } })
    ).toEqual('decide-fallback-value')
  ...
  it('falls back to decide', async () => {
    ...
    expect(await analytickit.getFeatureFlag('beta-feature', 'some-distinct-id')).toEqual('alakazam')
    expect(mockedUndici.fetch).toHaveBeenCalledTimes(1)
    mockedUndici.fetch.mockClear()

    // # beta-feature2 fallbacks to decide because region property not given with call
    expect(await analytickit.getFeatureFlag('beta-feature2', 'some-distinct-id')).toEqual('alakazam2')
    expect(mockedUndici.fetch).toHaveBeenCalledTimes(1)
  })
  ...
  it('dont fall back to decide when local evaluation is set', async () => {
    ...
    expect(await analytickit.getFeatureFlag('beta-feature', 'some-distinct-id', { onlyEvaluateLocally: true })).toEqual(
      undefined
    )
    expect(await analytickit.isFeatureEnabled('beta-feature', 'some-distinct-id', { onlyEvaluateLocally: true })).toEqual(
      undefined
    )
    expect(mockedUndici.fetch).not.toHaveBeenCalled()
    ...
    // # but doesn't because only_evaluate_locally is true
    expect(await analytickit.getFeatureFlag('beta-feature2', 'some-distinct-id', { onlyEvaluateLocally: true })).toEqual(
      undefined
    )
    expect(await analytickit.isFeatureEnabled('beta-feature2', 'some-distinct-id', { onlyEvaluateLocally: true })).toEqual(
      undefined
    )
    expect(mockedUndici.fetch).not.toHaveBeenCalled()
  })
  ...
  it("doesn't return undefined when flag is evaluated successfully", async () => {
    ...
    expect(await analytickit.getFeatureFlag('beta-feature', 'some-distinct-id')).toEqual(false)
    expect(await analytickit.isFeatureEnabled('beta-feature', 'some-distinct-id')).toEqual(false)
    expect(mockedUndici.fetch).not.toHaveBeenCalled()

    // # beta-feature2 falls back to decide, and whatever decide returns is the value
    expect(await analytickit.getFeatureFlag('beta-feature2', 'some-distinct-id')).toEqual(false)
    expect(await analytickit.isFeatureEnabled('beta-feature2', 'some-distinct-id')).toEqual(false)
  ...
  it('returns undefined when decide errors out', async () => {
    ...
    expect(await analytickit.getFeatureFlag('beta-feature2', 'some-distinct-id')).toEqual(undefined)
    expect(await analytickit.isFeatureEnabled('beta-feature2', 'some-distinct-id')).toEqual(undefined)
    expect(mockedUndici.fetch).toHaveBeenCalledTimes(2)
  })
  ...
  it('experience continuity flags are not evaluated locally', async () => {
    ...
    expect(await analytickit.getFeatureFlag('beta-feature', 'some-distinct-id')).toEqual('decide-fallback-value')
    expect(mockedUndici.fetch).toHaveBeenCalledTimes(1)
  })
  ...
  it('get all flags with fallback', async () => {
    ...
    expect(await analytickit.getAllFlags('distinct-id')).toEqual({
  ...
  it('get all flags with fallback but only_locally_evaluated set', async () => {
    ...
    expect(await analytickit.getAllFlags('distinct-id', { onlyEvaluateLocally: true })).toEqual({
      'beta-feature': true,
      'disabled-feature': false,
    })
    expect(mockedUndici.fetch).not.toHaveBeenCalled()
  ...
  it('get all flags with no fallback', async () => {
    ...
    expect(await analytickit.getAllFlags('distinct-id')).toEqual({ 'beta-feature': true, 'disabled-feature': false })
    expect(mockedUndici.fetch).not.toHaveBeenCalled()
  })
  ...
  it('computes inactive flags locally as well', async () => {
    ...
    await analytickit.reloadFeatureFlags()

    expect(await analytickit.getAllFlags('distinct-id')).toEqual({ 'beta-feature': false, 'disabled-feature': true })
    expect(mockedUndici.fetch).not.toHaveBeenCalled()
  })
})
...
describe('consistency tests', () => {
  ...
  // # See https://github.com/AnalyticKit/analytickit/blob/master/analytickit/test/test_feature_flag.py#L627
  ...
  it('is consistent for simple flags', () => {
    ...
    results.forEach(async (result, index) => {
      ...
      const value = await analytickit.isFeatureEnabled('simple-flag', distinctId)
  ...
  it('is consistent for multivariate flags', async () => {
    ...
    results.forEach(async (result, index) => {
      const distinctId = `distinct_id_${index}`
      const value = await analytickit.getFeatureFlag('multivariate-flag', distinctId)
      expect(value).toBe(result)
    })
  })
})
