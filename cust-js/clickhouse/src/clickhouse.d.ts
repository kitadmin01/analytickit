...
declare class ClickHouse<ConstructorOptions extends ClickHouse.Options = ClickHouse.Options> {
    ...
    querying<
        ...
        RecordType extends ClickHouse.DefaultRecordType<
            ConstructorOptions,
            QueryOptions,
            unknown
        > = ClickHouse.DefaultRecordType<ConstructorOptions, QueryOptions, any>
...
declare namespace ClickHouse {
    ...
    type DefaultRecordType<ConstructorOptions, QueryOptions, Value> = QueryOptions extends { format: 'JSON' }
        ? Record<string, Value>
        : QueryOptions extends { format: 'JSONCompact' }
        ? Array<Value>
        : QueryOptions extends { format: StringResultFormat }
        ? null
        : ConstructorOptions extends { format: 'JSON' }
        ? Record<string, Value>
        : ConstructorOptions extends { format: 'JSONCompact' }
        ? Array<Value>
        : ConstructorOptions extends { format: StringResultFormat }
        ? null
        : QueryOptions extends { dataObjects: true }
        ? Record<string, Value>
        : ConstructorOptions extends { dataObjects: true }
        ? Record<string, Value>
        : Array<Value>

    // string is returned for any non-JSON format
    export type QueryResult<RecordType> = RecordType extends null ? string : ObjectQueryResult<RecordType>
}
