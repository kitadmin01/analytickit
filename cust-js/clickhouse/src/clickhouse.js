...
function httpResponseHandler(stream, reqParams, reqData, cb, response) {
    ...
    function errorHandler(e) {
        ...
        if (!cb || (cb && stream.listeners('error').length)) stream.emit('error', err)
        return cb && cb(err)
    }
    ...
    response.on('end', function () {
        ...
        if (
            ...
            (!contentType || contentType.indexOf('text/plain') === 0 || contentType.indexOf('text/html') === 0) // WTF: xenial - no content-type, precise - text/html
        ) {
...
function httpRequest(reqParams, reqData, cb) {
    ...
    var req = requestInstance.request(reqParams, httpResponseHandler.bind(this, stream, reqParams, reqData, cb))

    req.on('error', function (e) {
        // user should define callback or add event listener for the error event
        if (!cb || (cb && stream.listeners('error').length)) stream.emit('error', e)
        return cb && cb(e)
    })
...
ClickHouse.prototype.query = function (chQuery, options, cb) {
    ...
    if (chQuery.match(/^(?:SELECT|WITH|SHOW|WATCH|DESC|DESCRIBE|EXISTS\s+TABLE)/i)) {
        ...
        if (chQuery.match(/\s+VALUES\b/i)) {
            ...
            options.omitFormat = true
        } else if (chQuery.match(/INSERT\s+INTO\s+\S+\s+(?:\([^\)]+\)\s+)?SELECT/im)) {
