...
describe('insert data', function () {
    ...
    it('inserts some prepared data using stream', function (done) {
        ...
        var stream = ch.query('INSERT INTO t', { queryOptions: { database: dbName } }, function (err, result) {
            ...
            ch.query(
                'SELECT * FROM t',
                { syncParser: true, queryOptions: { database: dbName } },
                function (err, result) {
    ...
    it('creates a table 2', function (done) {
        var ch = new ClickHouse({ host: host, port: port, queryOptions: { database: dbName } })
        ch.query(
            'CREATE TABLE t2 (a UInt8, b Float32, x Nullable(String), z DateTime) ENGINE = Memory',
            function (err, result) {
                assert(!err)
    ...
    it('creates a table 3', function (done) {
        var ch = new ClickHouse({ host: host, port: port, queryOptions: { database: dbName } })
        ch.query(
            'CREATE TABLE t3 (a UInt8, b Float32, x Nullable(String), z DateTime) ENGINE = Memory',
            function (err, result) {
                assert(!err)
    ...
    it('piping data from csv file', function (done) {
        ...
        function processFileStream(fileStream) {
            var stream = ch.query(
                'INSERT INTO t3',
                { format: 'CSV', queryOptions: { database: dbName } },
                function (err, result) {
                    assert(!err, err)
    ...
    it('piping data from tsv file', function (done) {
        ...
        function processFileStream(fileStream) {
            var stream = ch.query(
                'INSERT INTO t3',
                { format: 'TabSeparated', queryOptions: { database: dbName } },
                function (err, result) {
                    assert(!err, err)
    ...
    it('creates a table 4', function (done) {
        var ch = new ClickHouse({ host: host, port: port, queryOptions: { database: dbName } })
        ch.query(
            'CREATE TABLE t4 (arrayString Array(String), arrayInt Array(UInt32)) ENGINE = Memory',
            function (err, result) {
                assert(!err)
    ...
    it('inserts array with format JSON using stream', function (done) {
        ...
        var stream = ch.query(
            'INSERT INTO t4',
            { queryOptions: { database: dbName }, format: 'JSONEachRow' },
            function (err, result) {
                assert(!err, err)

                ch.query(
                    'SELECT * FROM t4',
                    { syncParser: true, queryOptions: { database: dbName }, dataObjects: 'JSON' },
                    function (err, result) {
    ...
    it('select data with FORMAT clause', function () {
        var ch = new ClickHouse({ host: host, port: port })
        return ch.querying('SELECT * FROM t5 FORMAT Values', { queryOptions: { database: dbName } }).then((data) => {
    ...
    it('select data with GET method and format option', function () {
        var ch = new ClickHouse({ host: host, port: port, useQueryString: true })
        return ch
            .querying('SELECT * FROM t5', { queryOptions: { database: dbName }, format: 'Values' })
            .then((data) => {
                assert.equal(data, `(0,0,NULL,'1970-01-02 00:00:00'),(1,1.5,'1','2050-01-01 00:00:00')`)
            })
    })
