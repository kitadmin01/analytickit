...
class ThreadPool {
    ...
    runTask(task, options) {
        let { filename, name, workerInfo } = options;
...
class Piscina extends eventemitter_asyncresource_1.default {
    ...
    broadcastTask(task, transferList, filename, signal) {
        const promises = [];
        for (const workerInfo of __classPrivateFieldGet(this, _Piscina_pool, "f").workers) {
            promises.push(__classPrivateFieldGet(this, _Piscina_pool, "f").runTask(task, { transferList, filename, signal, workerInfo }));
        }
