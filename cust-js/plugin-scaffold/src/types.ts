...
/** Input for a AnalyticKit plugin. */
...
/** A AnalyticKit plugin. */
export interface Plugin<Input extends PluginInput = {}> {
    /** Ran when the plugin is loaded by the AnalyticKit plugin server. */
    setupPlugin?: (meta: Meta<Input>) => void
    /** Ran when the plugin is unloaded by the AnalyticKit plugin server. */
...
/** Raw event received by AnalyticKit ingestion pipeline */
...
/** Event after being processed by AnalyticKit ingestion pipeline. */
...
/** NB: The following should replace DummyAnalyticKit in plugin-server/src/worker/vm/extensions/analytickit.ts */
