...
That said, we still build a version meant to be dropped into the browser which resides on _window.Simmer_ as before.
...
expect(simmer(el)).to.equal('#SomeElement')
```

When using the distribution located in the repo under **dist/simmer.js** a global _Simmer_ function will be exposed on the **window**.
...
const reconfiguredSimmer = simmer.configure({
  /* some custom configuration */
})
```
