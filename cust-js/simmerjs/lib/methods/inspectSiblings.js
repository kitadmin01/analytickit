...
function _default(hierarchy, state) {
  return hierarchy.reduce((selectorState, currentElem, index) => {
    const validClasses = (0, _lodash.default)(currentElem.getClasses(), 10)
      .filter(_validationHelpers.className)
      .map(
        (className) => `.${(0, _validationHelpers.escapeClassName)(className)}`
      )
