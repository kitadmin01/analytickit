...
function isUniqueDataAttr(query, key, value) {
  // use selector to query an element and see if it is a one-to-one selection
  ...
  return {
    ...
    getClasses: function () {
      return this.getClass()
        .split(' ')
        .map((className) =>
          className.replace(/^\s\s*/, '').replace(/\s\s*$/, '')
        )
...
function _default(scope, configuredQueryEngine) {
  const queryEngine =
    typeof configuredQueryEngine === 'function'
      ...
      : documentQuerySelector(scope) // If no selector we return an empty array - no CSS selector will ever be generated in this situation!
