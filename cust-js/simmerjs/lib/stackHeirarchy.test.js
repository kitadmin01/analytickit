...
const createElementHeirarchy = function () {
  ...
  return new JSDOM(`<body>${dom}</body>`).window.document.querySelector(
    selector
  )
}
...
describe('stackHierarchy', () => {
  test(`takes a wrapped element and a depth and returns the element heirarchy up tp that depth`, function () {
    ...
    const stackedHeirarchy = (0, _stackHierarchy.default)(
      (0, _queryEngine.wrap)(child),
      3
    )
  ...
  test(`throws an error if the depth is invalid`, function () {
    ...
    expect(() =>
      (0, _stackHierarchy.default)((0, _queryEngine.wrap)(child), 0)
    ).toThrowError(/An invalid depth of/)
    expect(() =>
      (0, _stackHierarchy.default)((0, _queryEngine.wrap)(child), -5)
    ).toThrowError(/An invalid depth of/)
  })
