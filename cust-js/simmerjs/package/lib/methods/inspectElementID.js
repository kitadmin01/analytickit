...
function _default(hierarchy, state, validateSelector, config, query) {
  return hierarchy.reduce((selectorState, currentElem, index) => {
    if (!selectorState.verified) {
      const [validatedState] = [currentElem.el.getAttribute('id')].filter(id => (0, _validationHelpers.attr)(id)) // make sure the ID is unique
      .filter(id => (0, _queryEngine.isUniqueElementID)(query, id)).map(validId => {
