...
var _lodash2 = _interopRequireDefault(require("lodash.flatmap"));
...
function analyzeElementSiblings(element, siblings) {
  return (0, _lodash.default)(element.getClasses(), (0, _lodash2.default)(siblings, sibling => sibling.getClasses())).length > 0 || not(getNodeNames(siblings).includes(element.el.nodeName));
}

function _default(hierarchy, state, validateSelector) {
  return hierarchy.reduce((selectorState, currentElem, index) => {
    if (!selectorState.verified) {
      ...
      const nextSiblings = currentElem.nextAll(); // get element's index by the number of elements before it
      // note that the nth-child selector uses a 1 based index, not 0

      const indexOfElement = prevSiblings.length + 1; // If the element has no siblings!
      // we have no need for the nth-child selector

      if ((prevSiblings.length || nextSiblings.length) && !analyzeElementSiblings(currentElem, [...prevSiblings, ...nextSiblings])) {
