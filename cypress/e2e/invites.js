...
describe('Invite Signup', () => {
    it('Authenticated user can invite user but cannot use invite for someone else', () => {
        ...
        cy.get('.Toastify__toast-body').should('contain', 'Invite for charlie@analytickit.com has been canceled')
        cy.get('[data-attr=invites-table] tbody td').should('not.contain', 'charlie@analytickit.com')
    })
    ...
    it('New user can use invite', () => {
        ...
        cy.get('#email').should('have.value', `n**********${target_email[11]}@analytickit.com`)
    ...
    it('can navigate to organization settings and invite/change users', () => {
        ...
        cy.get('[data-attr=invite-email-input]').type(`fake+${Math.floor(Math.random() * 10000)}@analytickit.com`)
