...
class TestPropDenormalized(ClickhouseTestMixin, BaseTest):
    ...
    def test_prop_person_denormalized(self):
        ...
        filter = Filter(
            data={
                "properties": [{"key": "email", "type": "person", "value": "analytickit", "operator": "not_icontains"}],
            }
        )
...
def test_parse_groups_persons_edge_case_with_single_filter(snapshot):
    filter = Filter(
        data={
            "properties": {"type": "OR", "values": [{"key": "email", "type": "person", "value": "1@analytickit.com"}],}
        }
    )
    assert (
        parse_prop_grouped_clauses(
...
def test_events(db, team) -> List[UUID]:
    return [
        _create_event(
            event="$pageview", team=team, distinct_id="whatever", properties={"email": "test@analytickit.com"},
        ),
