...
from dpa.tasks.subscriptions import (
    deliver_subscription_report,
    handle_subscription_value_change,
    schedule_all_subscriptions,
)
from dpa.tasks.test.subscriptions.subscriptions_test_factory import create_subscription


@patch("dpa.tasks.subscriptions.send_slack_subscription_report")
@patch("dpa.tasks.subscriptions.send_email_subscription_report")
@patch("dpa.tasks.subscriptions.generate_assets")
...
class TestSubscriptionsTasks(APIBaseTest):
    ...
    @patch("dpa.tasks.subscriptions.deliver_subscription_report")
    ...
    def test_deliver_subscription_report_email(
        ...
        assert mock_send_email.call_args_list == [
            call("test1@analytickit.com", subscription, [self.asset], invite_message=None, total_asset_count=1),
            call("test2@analytickit.com", subscription, [self.asset], invite_message=None, total_asset_count=1),
        ]
    ...
    def test_handle_subscription_value_change_email(
        ...
        subscription = create_subscription(
            ...
            target_value="test_existing@analytickit.com,test_new@analytickit.com",
        )
        mock_gen_assets.return_value = [self.insight], [self.asset]

        handle_subscription_value_change(
            subscription.id, previous_value="test_existing@analytickit.com", invite_message="My invite message"
        )
