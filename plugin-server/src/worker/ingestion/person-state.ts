...
console.error('handleIdentifyOrAlias failed', e, this.event)
    ...
    private async merge(
        ...
        // No reason to alias person against itself. Done by analytickit-node when updating user properties
...
await this.db.kafkaProducer.queueMessages(kafkaMessages)
    ...
    private async handleTablesDependingOnPersonID(
        ...
        await this.db.postgresQuery(
            'UPDATE analytickit_cohortpeople SET person_id = $1 WHERE person_id = $2',
